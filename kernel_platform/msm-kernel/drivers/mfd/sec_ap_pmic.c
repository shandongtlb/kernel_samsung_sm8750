...
static ssize_t show_fail_stats_info(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int i;
	struct qcom_stats_info *fail_stats = get_fail_stats();
	ssize_t info_size = 0;

	for (i = 0; i < SOC_STATS_COUNT; i++) {
		default_scnprintf(buf, info_size, "\"TYPE\":\"%s\",", fail_stats->soc_info[i].name);
		default_scnprintf(buf, info_size, "\"DUR\":\"%u.%03u\",", 
			fail_stats->soc_info[i].duration_sec, fail_stats->soc_info[i].duration_msec);
	}

	for (i = 0; i < SUBSYSTEM_STATS_COUNT; i++) {
		default_scnprintf(buf, info_size, "\"TYPE\":\"%s\",", fail_stats->subsystem_info[i].name);
		default_scnprintf(buf, info_size, "\"DUR\":\"%u.%03u\",", 
			fail_stats->subsystem_info[i].duration_sec, fail_stats->subsystem_info[i].duration_msec);
	}
	buf[strlen(buf) - 1] = ' ';
...
static DEVICE_ATTR(fail_stats_info, 0440, show_fail_stats_info, NULL);
