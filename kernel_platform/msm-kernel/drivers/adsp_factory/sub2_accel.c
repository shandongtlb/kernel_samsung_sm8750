...
static ssize_t ref_angle_sub_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct adsp_data *data = dev_get_drvdata(dev);
	uint8_t cnt = 0;
	...
	mutex_lock(&data->accel_factory_mutex);
	adsp_unicast(NULL, 0, MSG_REF_ANGLE_SUB, 0, MSG_TYPE_GET_RAW_DATA);

	while (!(data->ready_flag[MSG_TYPE_GET_RAW_DATA] & 1 << MSG_REF_ANGLE_SUB) &&
		cnt++ < TIMEOUT_CNT)
		msleep(20);

	data->ready_flag[MSG_TYPE_GET_RAW_DATA] &= ~(1 << MSG_REF_ANGLE_SUB);
	mutex_unlock(&data->accel_factory_mutex);
	...
	pr_info("[FACTORY] %s - st %d/%d, akm %d/%d, lf %d/%d, hall %d/%d/%d(uT)\n",
		__func__, data->msg_buf[MSG_REF_ANGLE_SUB][0],
		data->msg_buf[MSG_REF_ANGLE_SUB][1],
		data->msg_buf[MSG_REF_ANGLE_SUB][2],
		data->msg_buf[MSG_REF_ANGLE_SUB][3],
		data->msg_buf[MSG_REF_ANGLE_SUB][4],
		data->msg_buf[MSG_REF_ANGLE_SUB][5],
		data->msg_buf[MSG_REF_ANGLE_SUB][6],
		data->msg_buf[MSG_REF_ANGLE_SUB][7],
		data->msg_buf[MSG_REF_ANGLE_SUB][8]);

	return snprintf(buf, PAGE_SIZE, "%d,%d\n",
		data->msg_buf[MSG_REF_ANGLE_SUB][0], result);
}

static ssize_t sub_angle_read_data_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct adsp_data *data = dev_get_drvdata(dev);
	uint8_t cnt = 0;
	...
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
		data->msg_buf[MSG_REF_ANGLE_SUB][0],
		data->msg_buf[MSG_REF_ANGLE_SUB][1],
		data->msg_buf[MSG_REF_ANGLE_SUB][2],
		data->msg_buf[MSG_REF_ANGLE_SUB][3],
		data->msg_buf[MSG_REF_ANGLE_SUB][4],
...
static DEVICE_ATTR(dhr_sensor_info, 0440,
	sub2_accel_dhr_sensor_info_show, NULL);
#endif
static DEVICE_ATTR(ref_angle, 0444, ref_angle_sub_show, NULL);
static DEVICE_ATTR(read_angle_data, 0444, sub_angle_read_data_show, NULL);
