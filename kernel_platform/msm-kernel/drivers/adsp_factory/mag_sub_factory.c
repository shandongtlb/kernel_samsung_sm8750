...
#ifdef CONFIG_SUPPORT_VIRTUAL_MAG
static ssize_t sub_mag_vmag_id_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct adsp_data *data = dev_get_drvdata(dev);
	uint8_t cnt = 0;

	adsp_unicast(NULL, 0, MSG_VMAG, 0, MSG_TYPE_GET_THRESHOLD);
	while (!(data->ready_flag[MSG_TYPE_GET_THRESHOLD] & 1 << MSG_VMAG) &&
		cnt++ < TIMEOUT_CNT)
		usleep_range(500, 550);

	data->ready_flag[MSG_TYPE_GET_THRESHOLD] &= ~(1 << MSG_VMAG);

	if (cnt >= TIMEOUT_CNT) {
		pr_err("[FACTORY] %s: Timeout!!!\n", __func__);
	} else {
		pr_info("[FACTORY] %s - mag id %d\n",
			__func__,
			data->msg_buf[MSG_VMAG][0]);
	}
...
#endif
#ifdef CONFIG_SUPPORT_VIRTUAL_MAG
static DEVICE_ATTR(vmag_id, 0444, sub_mag_vmag_id_show, NULL);
#endif
