...
#define SNS_SMP2P_INFO_MAX_IDX	100
...
enum {
	SNS_SMP2P_EVENT_IDX,
	SNS_AWAKING_EVENT_IDX,
	SNS_SMP2P_MAX_IDX,
};

struct sns_smp2p_info {
	uint32_t smp2p_info_idx;
	struct sns_smp2p_values smp2p_values[SNS_SMP2P_INFO_MAX_IDX];
};
#endif
...
struct adsp_data {
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_ANALOG_HALL_FACTORY)
	struct mutex analog_hall_mutex;
#endif
	...
	struct delayed_work light_main_copr_work;
	struct delayed_work light_sub_copr_work;
	int32_t copr_data[2][15];
	int32_t last_ap_copr_log_time;
	bool has_pending_main_copr_data;
	bool has_pending_sub_copr_data;
#endif
	...
	struct delayed_work ssc_wakelock_check_work;
#endif
#if IS_ENABLED(CONFIG_SSC_SMP2P_EVENT_DEBUG)
	struct sns_smp2p_info smp2p_info[SNS_SMP2P_MAX_IDX];
#endif
	...
	bool fssr_ignore;
	bool fssr_dump;
	bool enable_max_duration_crash;
};
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_ANALOG_HALL_FACTORY)
int analog_hall_factory_init(void);
void analog_hall_factory_exit(void);
#endif
#if IS_ENABLED(CONFIG_SUPPORT_ACCEL_HIGHG)
