...
#if IS_ENABLED(CONFIG_SUPPORT_DEVICE_MODE) || IS_ENABLED(CONFIG_HALL_NOTIFIER)
...
#define TIME_LEN	24
#define WAKELOCK_DURATION_MAX	100000	//100s
#define WAKELOCK_CHECK_POLLING_TIME	10000  //10s
...
#define SENSOR_DUMP_CNT 6
#define SENSOR_RDUMP_CNT 7
#elif IS_ENABLED(CONFIG_SUPPORT_DUAL_6AXIS)
#define SUPPORT_DUAL_SENSOR "DUAL_GYRO"
...
#if IS_ENABLED(CONFIG_SUPPORT_DEVICE_MODE) || IS_ENABLED(CONFIG_SUPPORT_VIRTUAL_OPTIC) || IS_ENABLED(CONFIG_HALL_NOTIFIER)
#define FLIP_HALL_NAME "flip"
#define MPP_BACKCOVER_HALL_NAME "back_cover"
static int32_t curr_mpp_cover_state;
#define MAIN_FLIP_HALL_NAME "flip1"
#define SUB_FLIP_HALL_NAME "flip2"
static int32_t curr_sub_flip_state;
static int32_t curr_main_flip_state;
...
enum sns_crash_mode {
	SC_MODE_PANIC = 1,
	SC_MODE_SSR_DUMP_ON = 2,
	SC_MODE_SSR_DUMP_OFF = 3,
	SC_MODE_SSR_DUMP_COMPLETE = 4,
	SC_MODE_FSSR_IGNORE_TRUE = 5,
	SC_MODE_FSSR_IGNORE_FALSE = 6,
	SC_MODE_RECOVERY_DISABLE_TRUE = 8,
	SC_MODE_RECOVERY_DISABLE_FALSE = 9,
	SC_MODE_WS_DURATION_CRASH_ENABLE = 10,
	SC_MODE_WS_DURATION_CRASH_DISABLE = 11,
	SC_MODE_MAX
};
...
int get_ssc_recovery_command(int type)
{
	struct adsp_data *data = adsp_get_struct_data();
	int ret = -1;

	if (data == NULL) {
		pr_err("[FACTORY] %s: data NULL\n", __func__);
		return -1;
	}

	switch (type) {
	case SNS_RECOVERY_SSR:
		ret = OPTION_TYPE_SSC_RECOVERY;
		break;
	case SNS_RECOVERY_SSR_DUMP_START:
		if (data->fssr_dump)
			ret = OPTION_TYPE_SSC_SSR_DUMP;
		break;
	case SNS_RECOVERY_SSR_DUMP_ON:
	case SNS_RECOVERY_SSR_DUMP_OFF:
	case SNS_RECOVERY_SSR_DUMP_COMPLETE:
		ret = OPTION_TYPE_SSC_SSR_DUMP;
		break;
	default:
		ret = OPTION_TYPE_SSC_RECOVERY;
...
{
	int32_t msg_buf[2] = {0, };
	int32_t msg = get_ssc_recovery_command(type);

	if (msg < 0) {
		pr_err("[FACTORY] %s: skip, type:%d\n", __func__, type);
		return;
	}
	msg_buf[0] = msg;
	msg_buf[1] = type;
...
void ssr_reason_call_back(char reason[], int len)
{
	...
	sprintf(ssr_history[ssr_idx], "[%ld%02d%02d %02d:%02d:%02d] ",
...
void sns_save_smp2p_info(struct sns_smp2p_values *smp2p_values)
{
	struct adsp_data *data = adsp_get_struct_data();
	struct sns_smp2p_info *smp2p_info;
	uint32_t index;

	if (smp2p_values->is_awaking_event)
		smp2p_info = &data->smp2p_info[SNS_AWAKING_EVENT_IDX];
	else
		smp2p_info = &data->smp2p_info[SNS_SMP2P_EVENT_IDX];

	if (++(smp2p_info->smp2p_info_idx) >= SNS_SMP2P_INFO_MAX_IDX)
		smp2p_info->smp2p_info_idx = 0;
	index = smp2p_info->smp2p_info_idx;
	smp2p_info->smp2p_values[index].val = smp2p_values->val;
	smp2p_info->smp2p_values[index].sns_reserved_id = smp2p_values->sns_reserved_id;
	smp2p_info->smp2p_values[index].msg_id = smp2p_values->msg_id;
	smp2p_info->smp2p_values[index].connect_id = smp2p_values->connect_id;
	smp2p_info->smp2p_values[index].is_fac = smp2p_values->is_fac;
	smp2p_info->smp2p_values[index].is_sns_reserved_id = smp2p_values->is_sns_reserved_id;
	smp2p_info->smp2p_values[index].is_awaking_event = smp2p_values->is_awaking_event;
	ktime_get_real_ts64(&(smp2p_info->smp2p_values[index].ts));
}
EXPORT_SYMBOL_GPL(sns_save_smp2p_info);

void print_smp2p_values(struct sns_smp2p_info *smp2p_info, int idx)
{
	struct tm tm;

	time64_to_tm(smp2p_info->smp2p_values[idx].ts.tv_sec, 0, &tm);
	pr_cont("([%ld%02d%02d %02d:%02d:%02d]:(%d,%d,%d,%d,%d,%d,%d),",
		tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
		tm.tm_hour, tm.tm_min, tm.tm_sec,
		smp2p_info->smp2p_values[idx].val,
		smp2p_info->smp2p_values[idx].sns_reserved_id,
		smp2p_info->smp2p_values[idx].msg_id,
		smp2p_info->smp2p_values[idx].connect_id,
		smp2p_info->smp2p_values[idx].is_fac,
		smp2p_info->smp2p_values[idx].is_sns_reserved_id,
		smp2p_info->smp2p_values[idx].is_awaking_event);
}

void print_smp2p_info_events(void)
{
	struct adsp_data *data = adsp_get_struct_data();
	struct timespec64 ts;
	struct tm tm;
	...
	ktime_get_real_ts64(&ts);
	time64_to_tm(ts.tv_sec, 0, &tm);
	pr_info("%s, cur ts: %ld%02d%02d %02d:%02d:%02d\n", __func__,
		tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
		tm.tm_hour, tm.tm_min, tm.tm_sec);
	for (i = 0 ; i < 2; i++) {
		struct sns_smp2p_info *smp2p_info = &data->smp2p_info[i];
		int index = smp2p_info->smp2p_info_idx;

		pr_info("%s, type:%d\n", __func__, i);
		for (j = 0; j < index ; j++) {
			print_smp2p_values(smp2p_info, j);
			if (j != 0 && j % 10 == 0)
				pr_info("\n");
		}
	}
...
bool ssc_get_fssr_ignore(void)
{
	...
	return data->fssr_ignore;
}
EXPORT_SYMBOL_GPL(ssc_get_fssr_ignore);
...
int sns_hall_notify(struct notifier_block *nb,
	unsigned long state, void *v)
{
	struct hall_notifier_context *hall_notifier = v;
	int32_t msg_buf[3]; /* [0]: OPTION_TYPE, [1]: DEVICE_MODE_TYPE, [2]: OPEN/CLOSE */

	pr_info("[FACTORY] %s - name = %s, state = %s\n", __func__, hall_notifier->name, state ? "close" : "open");
	msg_buf[0] = OPTION_TYPE_SSC_DEVICE_MODE;

	if (!strncmp(hall_notifier->name, MPP_BACKCOVER_HALL_NAME, 9)) {
		msg_buf[1] = DEVICE_MODE_RESERVED_FOR_MPP_COVER;
		msg_buf[2] = state;
		curr_mpp_cover_state = (int32_t)state;
		adsp_unicast(msg_buf, sizeof(msg_buf), MSG_SSC_CORE, 0, MSG_TYPE_OPTION_DEFINE);
		pr_info("[FACTORY] %s - send mpp cover state %s\n", __func__, state ? "close" : "open");
	}
#if IS_ENABLED(CONFIG_SUPPORT_DEVICE_MODE)
	else if (!strcmp(hall_notifier->name, FLIP_HALL_NAME)) {
		struct adsp_data *data = container_of(nb, struct adsp_data, adsp_nb);

		msg_buf[1] = DEVICE_MODE_FLIP_OPEN;
		pr_info("[FACTORY] %s - before device mode curr:%s, fstate:%s\n",
			__func__, curr_fstate ? "close" : "open", data->fac_fstate ? "close" : "open");

		curr_fstate = (int32_t)state;
#if !defined(CONFIG_SEC_FACTORY) || !defined(CONFIG_SUPPORT_SENSOR_FLIP_MODEL)
		data->fac_fstate = curr_fstate;
#endif
		pr_info("[FACTORY] %s - after device mode curr:%s, fstate:%s\n",
			__func__, curr_fstate ? "close" : "open", data->fac_fstate ? "close" : "open");

		msg_buf[2] = curr_fstate;
		adsp_unicast(msg_buf, sizeof(msg_buf), MSG_SSC_CORE, 0, MSG_TYPE_OPTION_DEFINE);

		queue_work(pdata_ssc_flip->ssc_flip_wq, &pdata_ssc_flip->work_ssc_flip);
	} else if (!strcmp(hall_notifier->name, SUB_FLIP_HALL_NAME)) {
		msg_buf[1] = DEVICE_MODE_RESERVED_FOR_SUB_FLIP_HALL;
		msg_buf[2] = state;
		curr_sub_flip_state = (int32_t)state;
		adsp_unicast(msg_buf, sizeof(msg_buf), MSG_SSC_CORE, 0, MSG_TYPE_OPTION_DEFINE);
		pr_info("[FACTORY] %s - send sub_flip_hall state %s\n", __func__, state ? "close" : "open");
	} else if (!strcmp(hall_notifier->name, MAIN_FLIP_HALL_NAME)) {
		struct adsp_data *data = container_of(nb, struct adsp_data, adsp_nb);

		msg_buf[1] = DEVICE_MODE_RESERVED_FOR_MAIN_FLIP_HALL;
		msg_buf[2] = state;
		curr_main_flip_state = (int32_t)state;
		adsp_unicast(msg_buf, sizeof(msg_buf), MSG_SSC_CORE, 0, MSG_TYPE_OPTION_DEFINE);
		...
		data->fac_fstate = curr_fstate;
#endif
		pr_info("[FACTORY] %s - send main_flip_hall state %s, fstate %s\n",
			__func__, state ? "close" : "open", data->fac_fstate ? "close" : "open");
...
void sns_mpp_cover_init_work(void)
{
	...
	msg_buf[0] = OPTION_TYPE_SSC_DEVICE_MODE;
	msg_buf[1] = DEVICE_MODE_RESERVED_FOR_MPP_COVER;
	msg_buf[2] = curr_mpp_cover_state;
	if (curr_mpp_cover_state) {
		adsp_unicast(msg_buf, sizeof(msg_buf), MSG_SSC_CORE, 0, MSG_TYPE_OPTION_DEFINE);
		pr_info("[FACTORY] %s - send mpp cover state %s\n", __func__, curr_mpp_cover_state ? "close" : "open");
	}
}

void sns_multi_fold_flip_init_work(void)
{
	int32_t msg_buf[3]; /* [0]: OPTION_TYPE, [1]: DEVICE_MODE_TYPE, [2]: OPEN/CLOSE */

	msg_buf[0] = OPTION_TYPE_SSC_DEVICE_MODE;
	msg_buf[1] = DEVICE_MODE_RESERVED_FOR_SUB_FLIP_HALL;
	msg_buf[2] = curr_sub_flip_state;
	if (curr_sub_flip_state) {
		adsp_unicast(msg_buf, sizeof(msg_buf), MSG_SSC_CORE, 0, MSG_TYPE_OPTION_DEFINE);
		pr_info("[FACTORY] %s - send sub flip state %s\n", __func__, curr_sub_flip_state ? "close" : "open");
	}

	msg_buf[1] = DEVICE_MODE_RESERVED_FOR_MAIN_FLIP_HALL;
	msg_buf[2] = curr_main_flip_state;
	if (curr_main_flip_state) {
		adsp_unicast(msg_buf, sizeof(msg_buf), MSG_SSC_CORE, 0, MSG_TYPE_OPTION_DEFINE);
		pr_info("[FACTORY] %s - send main flip state %s\n", __func__, curr_main_flip_state ? "close" : "open");
	}
}
#endif
...
void ssc_flip_work_func(struct work_struct *work)
{
	...
	pr_info("[SSC_FAC] %s: device mode : %s\n", __func__, curr_fstate ? "close" : "open");
#if !defined(CONFIG_SEC_FACTORY) || !defined(CONFIG_SUPPORT_SENSOR_FLIP_MODEL)
	adsp_unicast(msg_buf, sizeof(msg_buf), MSG_VIR_OPTIC, 0, MSG_TYPE_OPTION_DEFINE);
#endif
#else
	msg_buf[0] = 11;
	msg_buf[1] = (int32_t)curr_fstate;
	pr_info("[SSC_FAC] %s: device mode : %s\n", __func__, curr_fstate ? "close" : "open");
	adsp_unicast(msg_buf, sizeof(msg_buf), MSG_LIGHT, 0, MSG_TYPE_OPTION_DEFINE);
#endif
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	adsp_unicast(fstate, sizeof(fstate), MSG_VIR_OPTIC, 0, MSG_TYPE_OPTION_DEFINE);

	pr_info("[SSC_FAC] %s - Factory flip state:%d", __func__, fstate[1]);
...
static void print_sensor_dump(struct adsp_data *data, int sensor)
{
	...
	case MSG_ACCEL_SUB:
		...
		break;
	case MSG_MAG:
		pr_info("[FACTORY] %s - %d: [00h-03h] %02x,%02x,%02x,%02x [10h-16h,18h] %02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x [30h-32h] %02x,%02x,%02x [cal_id] %d\n",
			__func__, sensor,
	...
	case MSG_PRESSURE:
		...
		break;
	case MSG_LIGHT:
		pr_info("[FACTORY] %s - %d(%s): %d,%d,%d,%d,%d,%d,%d [P]:%d,%d,%d [L]:%d,%d,%d [E]:%d,%d,%d [BR]:%d,%d\n",
			__func__, sensor, data->light_device_name[0],
			data->msg_buf[sensor][0], data->msg_buf[sensor][1],
			data->msg_buf[sensor][2], data->msg_buf[sensor][3],
			data->msg_buf[sensor][4], data->msg_buf[sensor][5],
			data->msg_buf[sensor][6], data->msg_buf[sensor][7],
			...
			data->msg_buf[sensor][12], data->msg_buf[sensor][13],
			data->msg_buf[sensor][14], data->msg_buf[sensor][15],
			data->max_brightness[0], data->brightness_resolution[0]);
#if IS_ENABLED(CONFIG_SUPPORT_LIGHT_CALIBRATION) && IS_ENABLED(CONFIG_SUPPORT_PROX_CALIBRATION)
		pr_info("[FACTORY] %s - %d: [L] R:%d, %d, %d, W:%d, ACL:%d, P:%d\n",
			__func__, sensor, data->light_cal_result,
			data->light_cal1, data->light_cal2,
			data->copr_w % 1000, data->copr_w / 1000, data->prox_cal);
#endif
		break;
#if IS_ENABLED(CONFIG_SUPPORT_DUAL_OPTIC)
	case MSG_LIGHT_SUB:
		pr_info("[FACTORY] %s - %d(%s): %d,%d,%d,%d,%d,%d,%d [P]:%d,%d,%d [L]:%d,%d,%d [E]:%d,%d,%d [BR]:%d,%d\n",
			__func__, sensor, data->light_device_name[1],
			data->msg_buf[sensor][0], data->msg_buf[sensor][1],
			data->msg_buf[sensor][2], data->msg_buf[sensor][3],
			data->msg_buf[sensor][4], data->msg_buf[sensor][5],
			data->msg_buf[sensor][6], data->msg_buf[sensor][7],
			...
			data->msg_buf[sensor][12], data->msg_buf[sensor][13],
			data->msg_buf[sensor][14], data->msg_buf[sensor][15],
			data->max_brightness[1], data->brightness_resolution[1]);
#if IS_ENABLED(CONFIG_SUPPORT_LIGHT_CALIBRATION) && IS_ENABLED(CONFIG_SUPPORT_PROX_CALIBRATION)
		pr_info("[FACTORY] %s - %d: [L] R:%d, %d, %d, W:%d, ACL:%d, P:%d\n",
			__func__, sensor, data->sub_light_cal_result,
			data->sub_light_cal1, data->sub_light_cal2,
			data->sub_copr_w % 1000, data->sub_copr_w / 1000, data->prox_sub_cal);
#endif
#endif
		break;
#if IS_ENABLED(CONFIG_SUPPORT_AK09973)
	...
	case MSG_SEQ_FOLD_MON:
		pr_info("[FACTORY] %s - %d: event: %d/%d subdhall/ahall/ang: %d/%d/%d lid_debug: %d/%d/%d/%d, sent_event: %d, subdhallgpio: %d, ahallgpio: %d\n",
			__func__, sensor,
			data->msg_buf[MSG_SEQ_FOLD_MON][0], data->msg_buf[MSG_SEQ_FOLD_MON][1],
			data->msg_buf[MSG_SEQ_FOLD_MON][2], data->msg_buf[MSG_SEQ_FOLD_MON][3],
			data->msg_buf[MSG_SEQ_FOLD_MON][4], data->msg_buf[MSG_SEQ_FOLD_MON][5],
			data->msg_buf[MSG_SEQ_FOLD_MON][6], data->msg_buf[MSG_SEQ_FOLD_MON][7],
			data->msg_buf[MSG_SEQ_FOLD_MON][8], data->msg_buf[MSG_SEQ_FOLD_MON][9],
			data->msg_buf[MSG_SEQ_FOLD_MON][10], data->msg_buf[MSG_SEQ_FOLD_MON][11]);
		break;
#endif
	default:
		break;
...
void sensor_dump_work_func(struct work_struct *work)
{
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_TRIPLE_6AXIS)
	int sensor_type[SENSOR_DUMP_CNT] = { MSG_LIGHT, MSG_ACCEL_SUB2, MSG_ACCEL_SUB, MSG_ACCEL, MSG_MAG, MSG_PRESSURE };
#elif IS_ENABLED(CONFIG_SUPPORT_DUAL_6AXIS)
	int sensor_type[SENSOR_DUMP_CNT] = { MSG_LIGHT, MSG_ACCEL_SUB, MSG_ACCEL, MSG_MAG, MSG_PRESSURE };
#else
	int sensor_type[SENSOR_DUMP_CNT] = { MSG_LIGHT, MSG_ACCEL, MSG_MAG, MSG_PRESSURE };
#endif
	...
	for (i = 0; i < SENSOR_DUMP_CNT; i++) {
		...
		if (sensor_type[i] == MSG_LIGHT) {
			...
			pr_info("[FACTORY] %s: Main - %s, Sub - %s\n", __func__,
				data->light_device_name[0], data->light_device_name[1]);
		}
#endif
	}
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_TRIPLE_6AXIS) || defined(CONFIG_SUPPORT_TRIPLE_6AXIS)
	sensor_get_dhr_info(data, MSG_SEQ_FOLD_MON);
#endif
	adsp_unicast(msg_buf, sizeof(msg_buf),
		MSG_SSC_CORE, 0, MSG_TYPE_OPTION_DEFINE);
...
#define LEN_PREFIX 15 /* "@@TYPE:%d##\n" */
#define DUMP_BUF_SIZE ((MAX_DUMP_CNT * LEN_REGISTER + LEN_PREFIX) * SENSOR_DUMP_CNT)
#define RBUF_SIZE (MAX_DUMP_CNT * LEN_REGISTER + LEN_PREFIX + LEN_EXTRA)
	...
	struct device_attribute *attr, char *buf)
{
	...
	int size;
#if IS_ENABLED(CONFIG_SUPPORT_TRIPLE_6AXIS)
		int sensor_type[SENSOR_RDUMP_CNT] = { MSG_ACCEL, MSG_MAG, MSG_ACCEL, MSG_LIGHT, MSG_PRESSURE, MSG_ACCEL_SUB, MSG_ACCEL_SUB2 };
		int arr_handle[SENSOR_RDUMP_CNT] = {
				...
				SENSOR_HANDLE_ACCELEROMETER_SUB2 };
#elif IS_ENABLED(CONFIG_SUPPORT_DUAL_6AXIS)
	...
	dump_buff = kzalloc(sizeof(char) * DUMP_BUF_SIZE, GFP_KERNEL);
	if (dump_buff == NULL) {
		pr_err("[FACTORY]: %s - dump_buff alloc failed\n", __func__);
		return -ENOMEM;
	}
	rbuf = kzalloc(sizeof(char) * RBUF_SIZE, GFP_KERNEL);
	if (rbuf == NULL) {
		pr_err("[FACTORY]: %s - rbuf alloc failed\n", __func__);
		kfree(dump_buff);
		return -ENOMEM;
	}

	mutex_lock(&data->sensors_dump_mutex);
	if (sensor_handle == -1) {
		...
		case SENSOR_HANDLE_ALL:
			for (int i = 0; i < SENSOR_RDUMP_CNT; i++) {
				...
				strlcat(dump_buff, rbuf, DUMP_BUF_SIZE);
			}
			break;
		default:
		...
		sensor_handle = -1;
		size = snprintf(buf, PAGE_SIZE, "%s%s%s", prefix, dump_buff, prefix);
	}
	mutex_unlock(&data->sensors_dump_mutex);
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_TRIPLE_6AXIS)
	} else if (strcmp(name, "accelerometer_sub2") == 0) {
		sensor_handle = SENSOR_HANDLE_ACCELEROMETER_SUB2;
		sensor_idx = MSG_ACCEL_SUB2;
#endif
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[FACTORY] %s : %d\n", __func__, mode);
	if (mode == SC_MODE_PANIC) {
		panic("sensor is abnormal, force panic\n");
	} else if (mode == SC_MODE_SSR_DUMP_ON
		|| mode == SC_MODE_SSR_DUMP_OFF
		|| mode == SC_MODE_SSR_DUMP_COMPLETE) {
		if (mode == SC_MODE_SSR_DUMP_COMPLETE) {
			schedule_delayed_work(&ssr_dump_work, msecs_to_jiffies(7000));
		} else {
			int32_t msg_buf[2] = {0, };

			data->fssr_dump = (mode == SC_MODE_SSR_DUMP_ON) ? true : false;
			msg_buf[0] = OPTION_TYPE_SSC_SSR_DUMP;
			msg_buf[1] = (mode == SC_MODE_SSR_DUMP_ON) ?
				SNS_RECOVERY_SSR_DUMP_ON:SNS_RECOVERY_SSR_DUMP_OFF;
			pr_info("[FACTORY] %s: msg_buf = %d,%d\n",
				__func__, msg_buf[0], msg_buf[1]);
			adsp_unicast(msg_buf, sizeof(msg_buf),
					MSG_SSC_CORE, 0, MSG_TYPE_OPTION_DEFINE);
		}
	} else if (mode == SC_MODE_FSSR_IGNORE_TRUE
		|| mode == SC_MODE_FSSR_IGNORE_FALSE) {
		if (data == NULL) {
			pr_info("[FACTORY] %s, adsp remoteproc null\n", __func__);
			goto crash_return;
		}

		data->fssr_ignore = (mode == SC_MODE_FSSR_IGNORE_TRUE) ? true:false;
		pr_info("[FACTORY] %s, fssr_ignore:%d\n",
			__func__, (int)data->fssr_ignore);			
	} else if (mode == SC_MODE_RECOVERY_DISABLE_TRUE
		|| mode == SC_MODE_RECOVERY_DISABLE_FALSE) {
		struct rproc *adsp = get_adsp_remoteproc();
		...
		if (mode == SC_MODE_RECOVERY_DISABLE_TRUE) {
			adsp->recovery_disabled = true;
		} else {
			adsp->recovery_disabled = false;
		}
		pr_info("recovery_disabled:%d\n", (int)adsp->recovery_disabled);
	} else if (mode == SC_MODE_WS_DURATION_CRASH_ENABLE
		|| mode == SC_MODE_WS_DURATION_CRASH_DISABLE) {
		data->enable_max_duration_crash =
			(mode == SC_MODE_WS_DURATION_CRASH_ENABLE) ? true : false;
		pr_info("enable_max_duration_crash:%d\n",
			(int)data->enable_max_duration_crash);
	}
crash_return:
	return size;
}

static ssize_t sns_firm_idx_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct rproc *adsp = get_adsp_remoteproc();
	...
	pr_info("[FACTORY] %s, firm index:%d\n", __func__, adsp->index);
	return snprintf(buf, PAGE_SIZE, "%d\n", adsp->index);
}
...
{
	send_ssc_recovery_command(SNS_RECOVERY_SSR_DUMP_COMPLETE);
}

#if IS_ENABLED(CONFIG_SSC_WAKELOCK_DEBUG)
void ssc_wakelock_checker(struct adsp_data *data, struct wakeup_source *ws)
{
	...
	if (ws->active) {
		...
		if (duration_ms > WAKELOCK_DURATION_MAX &&
			data->enable_max_duration_crash) {
			pr_info("[FACTORY] a:%llu, t:%lld\n",
				duration_ms, total_time);
			pr_info("[FACTORY] ac:%lu, ec:%lu, wc:%lu, ex:%lu\n", 
				ws->active_count, ws->event_count,
				ws->wakeup_count, ws->expire_count);
			panic("smp2p-sleepstate has been acquired for too long");
		}
	}
}

void ssc_wakelock_check_work_func(struct work_struct *work)
{
	struct adsp_data *data = container_of((struct delayed_work *)work,
		struct adsp_data, ssc_wakelock_check_work);
	...
	for_each_wakeup_source(ws) {
		...
		if (!strncmp(ws->name, "smp2p-sleepstate", 16)) {
			ssc_wakelock_checker(data, ws);
		}
	}
	schedule_delayed_work(&data->ssc_wakelock_check_work,
		msecs_to_jiffies(WAKELOCK_CHECK_POLLING_TIME));
}
#endif
...
static DEVICE_ATTR(sns_firm_idx, 0440, sns_firm_idx_show, NULL);
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_DEVICE_MODE) || defined(CONFIG_VBUS_NOTIFIER) || IS_ENABLED(CONFIG_HALL_NOTIFIER)
	data->adsp_nb.notifier_call = sns_hall_notify,
	data->adsp_nb.priority = 1,
	pr_info("[FACTORY]: %s - hall_notifier_register\n", __func__);
...
#if IS_ENABLED(CONFIG_SSC_WAKELOCK_DEBUG)
	INIT_DELAYED_WORK(&data->ssc_wakelock_check_work, ssc_wakelock_check_work_func);	
#endif
#ifdef CONFIG_VBUS_NOTIFIER
