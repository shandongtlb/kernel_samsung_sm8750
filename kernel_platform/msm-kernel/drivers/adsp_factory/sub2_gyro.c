	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!data->msg_buf[MSG_GYRO_SUB2][5])
		...
		pr_info("[FACTORY] %s - cause: %d, time_spent: %d, stmc: 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X",
			__func__,
			data->msg_buf[MSG_GYRO_SUB2][16], data->msg_buf[MSG_GYRO_SUB2][17],
			(data->msg_buf[MSG_GYRO_SUB2][18] >> 24) & 0xFF,
			(data->msg_buf[MSG_GYRO_SUB2][18] >> 16) & 0xFF,
			(data->msg_buf[MSG_GYRO_SUB2][18] >> 8) & 0xFF,
			data->msg_buf[MSG_GYRO_SUB2][18] & 0xFF,
			data->msg_buf[MSG_GYRO_SUB2][19] & 0xFF);
		st_diff_res = ST_PASS;
	}
