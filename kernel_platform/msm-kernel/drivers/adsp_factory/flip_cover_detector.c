...
#define FCD_TIMEOUT_CNT_THD 3000 // 30 seconds
...
static uint32_t fcd_timeout_cnt;
...
static enum hrtimer_restart fcd_timer_func(struct hrtimer *timer)
{
	...
	// If factory application is not running but HR timer is running
	// due to disable command not received from factory app or other unexpected reasons,
	// then stop HR timer after checking for 30 seconds to avoid AP awake and prevent battery drainage
	if (fcd_timeout_cnt++ > FCD_TIMEOUT_CNT_THD) {
		pr_info("[FACTORY] %s: Stop HR Timer as factory app not running\n", __func__);
