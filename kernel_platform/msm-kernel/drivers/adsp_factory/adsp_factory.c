...
static u8 msg_size[MSG_SENSOR_MAX] = {
	...
	MSG_REF_ANGLE_MAX,
	MSG_SEQ_FOLD_MON_MAX,
#endif
#if IS_ENABLED(CONFIG_SUPPORT_VIRTUAL_MAG) || defined(CONFIG_SUPPORT_VIRTUAL_MAG)
	MSG_VMAG_MAX,
#endif
	...
	struct device_attribute *attributes[])
{
	...
	case MSG_ANALOG_HALL:
		dev_name = "analog_hall";
		break;
#endif
#if IS_ENABLED(CONFIG_FLIP_COVER_DETECTOR_FACTORY)
...
static int process_received_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	if (sensor_type == MSG_FACTORY_INIT_CMD) {
		...
		pr_info("[FACTORY] %s - MSG_SENSOR_MAX = %d\n", __func__, MSG_SENSOR_MAX);
...
static int __init factory_adsp_init(void)
{
	...
	data->enable_max_duration_crash = true;
	...
	INIT_DELAYED_WORK(&data->light_main_copr_work, light_main_copr_work_func);
	INIT_DELAYED_WORK(&data->light_sub_copr_work, light_sub_copr_work_func);
#endif
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_ANALOG_HALL_FACTORY)
	mutex_init(&data->analog_hall_mutex);
#endif
#if IS_ENABLED(CONFIG_SUPPORT_ACCEL_HIGHG)
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_ANALOG_HALL_FACTORY)
	mutex_destroy(&data->analog_hall_mutex);
#endif
#if IS_ENABLED(CONFIG_SUPPORT_AK09973)
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_AP_COPR_FOR_LIGHT_SENSOR)
	cancel_delayed_work_sync(&data->light_main_copr_work);
	cancel_delayed_work_sync(&data->light_sub_copr_work);
#endif
#if IS_ENABLED(CONFIG_SUPPORT_BRIGHTNESS_NOTIFY_FOR_LIGHT_SENSOR)
