...
static int max77775_get_ship_entry_db(struct max77775_charger_data *charger)
{
	u8 reg_data = 0, ship_entry_db = 0;
	int ret = 0;

	max77775_read_reg(charger->i2c, MAX77775_CHG_REG_CNFG_13, &reg_data);
	ship_entry_db = (reg_data & CHG_CNFG_13_SHIP_ENTRY_DB_MASK);
	pr_info("%s : CHG_CNFG_13(0x%02x), db_t(0x%02x)\n", __func__, reg_data, ship_entry_db);

	if (ship_entry_db == 0x00)
		ret = 0;
	else if (ship_entry_db == 0x01)
		ret = 16;
	else if (ship_entry_db == 0x02)
		ret = 32;
	else
		ret = 192;
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_SHIPMODE_TEST_DB_T:
			val->intval = max77775_get_ship_entry_db(charger);
			break;
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_PAD_VOLT_CTRL:
			break;
		case POWER_SUPPLY_EXT_PROP_SHIPMODE_TEST_DB_T:
			charger->pdata->ship_entry_db_time = val->intval;
