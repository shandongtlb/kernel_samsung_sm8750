...
/* mix temp v2 */
	...
	ret = of_property_read_u32(np, "battery,tx_ring_vout",
			&pdata->tx_ring_vout);
	if (ret) {
		pdata->tx_ring_vout = pdata->tx_uno_vout; // battery,tx_ring_vout in dt is not mandatory
		pr_info("%s : tx ring vout is Empty. set %d\n", __func__, pdata->tx_buds_vout);
	}
