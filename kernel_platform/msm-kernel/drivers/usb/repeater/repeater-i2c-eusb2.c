...
#define MAX_REPEATER_TYPE_STRING 10
...
#define TUNE_MAX_NXP 18
#define TUNE_MAX_TI 19
#define TUNE_MAX_DIODES 11
...
static u8 tune_map_diodes[TUNE_MAX_DIODES] = {
	DIODES_PI3EUSB1100_M_F_CONTROL,
	DIODES_PI3EUSB1100_USB2_TX_EQ_CONTROL,
	DIODES_PI3EUSB1100_USB2_TX_EQ_OUT_CURRENT_CONTROL,
	DIODES_PI3EUSB1100_USB2_RX_EQ_CONTROL,
	DIODES_PI3EUSB1100_USB2_RX_EQ_SSS_CONTROL,
	DIODES_PI3EUSB1100_USB2_SDO_CONTROL,
	DIODES_PI3EUSB1100_USB2_TX_OUT_SWING_CONTROL,
	DIODES_PI3EUSB1100_USB2_FS_OUT_DDSS_CONTROL,
	DIODES_PI3EUSB1100_REV_ID,
	DIODES_PI3EUSB1100_DEV_ID_LO,
	DIODES_PI3EUSB1100_DEV_ID_HI,
};
#endif
...
const char
eusb2_repeater_type_print[REPEATER_TYPE_MAX][MAX_REPEATER_TYPE_STRING] = {
	{"TI"},
	{"NXP"},
	{"DIODES"},
};
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&ter->er_tune_lock);
	sprintf(str, "\n Address Value - %s\n", eusb2_repeater_type_print[ter->chip->repeater_type]);
	...
	} else if (ter->chip->repeater_type == TI_REPEATER) {
	...
	} else if (ter->chip->repeater_type == DIODES_REPEATER_PI3EUSB1100) {
		for (i = 0; i < TUNE_MAX_DIODES; i++) {
			strcpy(str2, str);
			ret = eusb2_i2c_read_reg(ter, tune_map_diodes[i], &reg_val);
			if (ret < 0) {
				dev_err(ter->dev, "Failed to read reg:0x%02x ret=%d\n", tune_map_diodes[i], ret);
				mutex_unlock(&ter->er_tune_lock);
				return sprintf(buf, "Failed to read reg\n");
			}
			sprintf(str, "%s  0x%2x   0x%2x\n", str2, tune_map_diodes[i], reg_val);
		}
	} else {
		dev_err(ter->dev, "unknown repeater_type=%d\n", ter->chip->repeater_type);
		mutex_unlock(&ter->er_tune_lock);
		return sprintf(buf, "unknown repeater_type\n");
	}
	mutex_unlock(&ter->er_tune_lock);
...
static int eusb2_repeater_init(struct usb_repeater *ur)
{
	...
	dev_info(er->ur.dev, "%s %s mode\n",
		eusb2_repeater_type_print[er->chip->repeater_type], er->ur.is_host ? "HOST":"CLIENT");
...
static int eusb2_repeater_reset(struct usb_repeater *ur, bool bring_out_of_reset)
{
	...
	case DIODES_REPEATER_PI3EUSB1100:
		...
		break;
	case NXP_REPEATER:
		/* Reset pin for NXP repeater isn't actual reset.
		 * It is set to bypass mode if it is low.
		 * Keep this high because of current issue.
		 */
		dev_dbg(ur->dev, "NXP, skip reset\n");
		break;
	default:
		dev_err(er->ur.dev, "Invalid repeater\n");
	}
...
static int eusb2_repeater_powerdown(struct usb_repeater *ur)
{
	...
	ret = eusb2_repeater_power(er, false);

	if (chip->repeater_type == NXP_REPEATER) {
		/* Turn on regulator after 30ms because of current issue*/
		usleep_range(30000, 30000);
		dev_dbg(ur->dev, "Turn on regulator after 30ms\n");
		ret = eusb2_repeater_power(er, true);
	}
...
#endif
	pr_info("%s %s done\n", __func__, eusb2_repeater_type_print[er->chip->repeater_type]);
	return 0;
