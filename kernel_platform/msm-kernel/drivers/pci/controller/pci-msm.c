				       ...
				       unsigned long action, void *data)
{
	...
	PCIE_ERR(dev, "PCIe RC%d: %s notifier (action %lu%s)\n",
...
#ifdef CONFIG_SEC_PCIE
		PCIE_ERR(dev, "PCIe RC%d target_link_speed=0x%x\n",
...
int msm_pcie_enumerate(u32 rc_idx)
{
	...
	if (!bridge) {
		PCIE_ERR(dev, "PCIe: failed to alloc host bridge for RC%d\n",
				dev->rc_idx);
		ret = -ENOMEM;
		goto out;
	}
	...
	if (!dev->lpi_enable) {
		...
		if (ret) {
			PCIE_ERR(dev, "PCIe: failed to initialize msi for RC%d: %d\n",
					dev->rc_idx, ret);
			goto out;
		}
	}
...
static irqreturn_t handle_aer_irq(int irq, void *data)
{
...
#ifdef CONFIG_SEC_PCIE_SET_EP_STATUS
	if ((e_src.status & PCI_ERR_ROOT_COR_RCV) && !(e_src.status & PCI_ERR_ROOT_UNCOR_RCV)) {
		if (!dev->is_ep_enabled) {
			PCIE_INFO(dev, "PCIe: RC%d: Skip AER panic if EP is not enabled.\n",
				dev->rc_idx);
			dev->aer_irq_counter--;
		}
	}
	...
	if (dev->pcie_aer_det_gpio != -EINVAL)
		gpio_set_value(dev->pcie_aer_det_gpio, 1);
#endif
...
static void msm_pcie_create_ipc_logs(u32 rc_idx)
{
	...
	if (msm_pcie_dev[rc_idx].ipc_log == NULL ||
		...
		PCIE_DBG(&msm_pcie_dev[rc_idx],
			"%s: PCIe IPC logging is enable for RC%d\n",
			__func__, rc_idx);
#endif
}
...
#ifdef CONFIG_SEC_PANIC_PCIE_ERR
	ret = of_get_named_gpio(of_node, "pcie-aer-det-gpio", 0);
	if (ret >= 0) {
		pcie_dev->pcie_aer_det_gpio = ret;
		PCIE_ERR(pcie_dev, "RC%d pcie_aer_det_gpio:%d\n",
				pcie_dev->rc_idx, pcie_dev->pcie_aer_det_gpio);
		gpio_direction_output(pcie_dev->pcie_aer_det_gpio, 0);
	} else {
		pcie_dev->pcie_aer_det_gpio = -EINVAL;
		PCIE_ERR(pcie_dev, "RC%d pcie_aer_det_gpio is not defined.\n",
				pcie_dev->rc_idx);
	}
#endif
#endif
...
int msm_pcie_allow_l1(struct pci_dev *pci_dev)
{
	...
	if (pcie_dev->debugfs_l1) {
		PCIE_DBG2(pcie_dev,
			"[%d] PCIe: RC%d: debugfs_l1 is set so no-op\n",
	...
	if (pcie_dev->link_status != MSM_PCIE_LINK_ENABLED) {
		ret = -EINVAL;
		PCIE_DBG2(pcie_dev, "[%d] PCIe: RC%d: %02x:%02x.%01x: lnk_sts %d\n",
				current->pid, pcie_dev->rc_idx, pci_dev->bus->number,
		  ...
		  const struct pci_device_id *device_id)
{
	...
	ret = dma_set_mask_and_coherent(&pci_dev->dev, (u64)DMA_BIT_MASK(64));
	if (ret) {
...
#ifdef CONFIG_SEC_PCIE_SET_EP_STATUS
void sec_pcie_set_ep_enabled(struct pci_dev *dev, bool is_enabled)
{
	struct msm_pcie_dev_t *pcie_dev = PCIE_BUS_PRIV_DATA(dev->bus);
	pcie_dev->is_ep_enabled = is_enabled;
}
EXPORT_SYMBOL(sec_pcie_set_ep_enabled);
#endif
