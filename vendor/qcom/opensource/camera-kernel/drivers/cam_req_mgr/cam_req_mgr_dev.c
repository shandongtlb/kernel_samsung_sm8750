...
static int cam_req_mgr_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_SEC_Q7Q_PROJECT)
	...
	if (!gpio_get_value(UPPER_C2C_DET_GPIO)) {
		...
		if (IS_ERR_OR_NULL(rgltr)) {
	        	CAM_ERR(CAM_CRM, "deferring probe, Wait DIO8018 probe");
			rc = -EPROBE_DEFER;
	        	goto end;
		}
	}
#elif defined(CONFIG_SEC_Q7MQ_PROJECT)
	struct regulator *rgltr = NULL;
	...
	if (!gpio_get_value(UPPER_C2C_DET_GPIO)) {
		rgltr = devm_regulator_get_optional(dev,
			"ET5917_0_LDO7");
		if (IS_ERR_OR_NULL(rgltr)) {
	        	CAM_ERR(CAM_CRM, "deferring probe, Wait ET5917 probe");
			rc = -EPROBE_DEFER;
	        	goto end;
		}
	}
