...
int cam_req_mgr_process_error(void *priv, void *data)
{
	...
	case CRM_KMD_WARN_INTERNAL_RECOVERY:
		...
		if (idx < 0) {
			...
			slot = &in_q->slot[idx];
			if ((err_info->error == CRM_KMD_ERR_BUBBLE) && !slot->recover) {
				CAM_WARN(CAM_CRM,
	...
	struct cam_req_mgr_core_link *link)
{
	...
	CAM_INFO(CAM_CRM,"link->link_state_spin_lock locking");
	spin_lock_bh(&link->link_state_spin_lock);
	link->state = CAM_CRM_LINK_STATE_IDLE;
	spin_unlock_bh(&link->link_state_spin_lock);
	...
	if (!link->is_shutdown) {
		...
		CAM_INFO(CAM_CRM,"link->req.lock locking");
		mutex_lock(&link->req.lock);
	...
	CAM_INFO(CAM_CRM,"link->lock locking");
	mutex_lock(&link->lock);
	CAM_INFO(CAM_CRM,"link->lock locked and link->link_state_spin_lock locking");
	spin_lock_bh(&link->link_state_spin_lock);
	...
	CAM_INFO(CAM_CRM,"session->lock locking");
	mutex_lock(&session->lock);
	CAM_INFO(CAM_CRM,"session->lock locked");
	...
	for (i = 0; i < control->num_links; i++) {
		...
		if (control->ops == CAM_REQ_MGR_LINK_ACTIVATE) {
			...
			if (cam_clock_data_recovery_is_requested(1) ||
				cam_clock_data_recovery_is_requested(2))
				init_timeout = 1800;
#endif
