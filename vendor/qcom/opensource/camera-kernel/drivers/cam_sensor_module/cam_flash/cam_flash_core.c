	...
	struct cam_flash_frame_setting *flash_data, enum camera_flash_opcode op)
{
    ...
    if (flash_ctrl->sysfs_flash) {
        CAM_ERR(CAM_FLASH, "The camera led control is not allowed"
	...
	struct cam_flash_frame_setting *flash_data)
{
	...
	/* In 3rd party app torch-->High is issued without
	* switching OFF switch_trigger, so make sure Switch
	* switch trigger is called with LED_OFF. So that FLASH
	* HIGH will work */
...
ssize_t flash_power_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	//default value
	flash_data.led_current_ma[0] = 75;

	soc_private = (struct cam_flash_private_soc *)
		g_flash_ctrl->soc_info.soc_private;
	max_current = soc_private->torch_max_current[0];

	if (value == 0) // Torch OFF
		current_ma = 0;
	else if (value == 1) // Torch ON
		current_ma = 60;
	else if (value == 100 || value == 200) // Factory Torch ON
		current_ma = 240;
	else if (value == 1001) // level 1 (Flashlight level 1)
		current_ma = 40;
	else if (value == 1002) // level 2 (Flashlight level 2)
		current_ma = 60;
	else if (value == 1003) // level 3
		current_ma = 80;
	else if (value == 1004) // level 4 (Flashlight level 3)
		current_ma = 100;
	else if (value == 1005)	// level 5
		current_ma = 120;
	else if (value == 1006) // level 6 (Flashlight level 4)
		current_ma = 160;
	else if (value == 1007) // level 7
		current_ma = 180;
	else if (value == 1008) // level 8
		current_ma = 180;
	else if (value == 1009) // level 9 (Flashlight level 5)
		current_ma = 200;
	else if (value == 1010) // level 10
		current_ma = 200;
	else if ((2001 <= value) && (value <= 2015)) { // Torch ON for tunning : Step 20mA ~ 300mA
		current_ma = (value - 2000) * 20;
		CAM_INFO(CAM_FLASH, "torch current %u", current_ma);
	} else {
		CAM_ERR(CAM_FLASH, "value %u is invalid", value);
		current_ma = 0;
	}

	CAM_INFO(CAM_FLASH, "torch current %u, max_current %u",
		current_ma, max_current);

	if (current_ma == 0) {
		cam_flash_off(g_flash_ctrl);
		g_flash_ctrl->sysfs_flash = false;
		CAM_INFO(CAM_FLASH,"torch off");
	}

	if (current_ma > 0 && current_ma <= max_current) {
		cam_flash_off(g_flash_ctrl);
		flash_data.led_current_ma[0] = current_ma;
		flash_data.led_current_ma[1] = 0;
		flash_data.opcode = CAMERA_SENSOR_FLASH_OP_FIRELOW;
		cam_flash_low(g_flash_ctrl,&flash_data);
		g_flash_ctrl->sysfs_flash = true;
