...
#endif
#include "cam_sensor_debug.h"
#include "cam_sensor_debug_gn3.h"
#include "cam_sensor_debug_hp2.h"
#include "cam_sensor_debug_imx564_754.h"
#include "cam_sensor_debug_jn3.h"
		...
		case SENSOR_CUSTOM_REG2:
			...
			if (power_setting->seq_val < num_vreg) {
				...
				if (power_setting->seq_type == SENSOR_VANA ||
					power_setting->seq_type == SENSOR_VDIG)
				{
					cam_soc_util_regulator_set_mode(
						soc_info->rgltr[vreg_idx],
						soc_info->rgltr_name[vreg_idx],
						REGULATOR_MODE_FAST);
				}
...
#if defined(CONFIG_SEC_PA1Q_PROJECT) || defined(CONFIG_SEC_PA2Q_PROJECT) || defined(CONFIG_SEC_Q7Q_PROJECT) || defined(CONFIG_SEC_Q7MQ_PROJECT)
...
#if defined(CONFIG_SAMSUNG_CAMERA)
					if (pd->seq_type == SENSOR_VANA ||
						pd->seq_type == SENSOR_VDIG)
					{
						cam_soc_util_regulator_set_mode(
							soc_info->rgltr[ps->seq_val],
							soc_info->rgltr_name[ps->seq_val],
...
struct st_vc_addr {
	...
	[e_vc_addr_260_264_1b_read] = { 0x0260, 0x0264, CAMERA_SENSOR_I2C_TYPE_BYTE },
	[e_vc_addr_260_264_2b_read] = { 0x0260, 0x0264, CAMERA_SENSOR_I2C_TYPE_WORD },
	...
	[e_vc_addr_110] = { 0x0110, 0x0000, CAMERA_SENSOR_I2C_TYPE_BYTE }
};
...
inline uint32_t get_seq_sensor_id(uint32_t sensor_id)
{
	...
	case SENSOR_ID_S5KJN3:
		ret = e_seq_sensor_idn_s5kjn3;
		break;
	case SENSOR_ID_IMX874:
		ret = e_seq_sensor_idn_imx874;
		break;
	case SENSOR_ID_IMX825:
		ret = e_seq_sensor_idn_imx825;
		break;
	default:
...
inline int32_t get_vc_pick_idx(uint32_t sensor_id)
{
	...
	case SENSOR_ID_S5KGN3:
	case SENSOR_ID_S5K3LU:
		return e_vc_addr_260_264_1b_read;
	case SENSOR_ID_S5KJN3:
		return e_vc_addr_260_264_2b_read;
	case SENSOR_ID_S5KHP2:
...
uint32_t cam_sensor_dbg_get_dcg_value_hp2(struct cam_sensor_ctrl_t* s_ctrl)
{
	struct cam_sensor_i2c_reg_setting dbg_reg_page4000 = { 0 };
	struct cam_sensor_i2c_reg_setting dbg_reg_page1000 = { 0 };
	const uint32_t debug_addr_check_dcg = 0xCADE;

	int32_t rc = 0;
	uint32_t val = 0;
	uint32_t size = 0;

	size = ARRAY_SIZE(hp2_page_4000_reg_array);
	...
	if (dbg_reg_page4000.reg_setting != NULL) {
		...
		memcpy(dbg_reg_page4000.reg_setting, &hp2_page_4000_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
	}

	size = ARRAY_SIZE(hp2_page_1000_reg_array);
	dbg_reg_page1000.reg_setting = kmalloc(sizeof(struct cam_sensor_i2c_reg_array) * size, GFP_KERNEL);
	if (dbg_reg_page1000.reg_setting != NULL) {
		dbg_reg_page1000.size = size;
		dbg_reg_page1000.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
		dbg_reg_page1000.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
		dbg_reg_page1000.delay = 0;
		memcpy(dbg_reg_page1000.reg_setting, &hp2_page_1000_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
	}

	// page 1000
	rc = camera_io_dev_write(&s_ctrl->io_master_info, &dbg_reg_page1000);
	if (rc < 0)
		CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] Failed to write dbg_write_setting_1000 %d", rc);

	rc = camera_io_dev_read(
		&(s_ctrl->io_master_info),
		debug_addr_check_dcg,
	...
	if (rc < 0)
		...
		CAM_DBG(CAM_SENSOR_UTIL, "[SEN_DBG] addr:0x%4x val:0x%4x",
			debug_addr_check_dcg, val);
	}
	...
	if (dbg_reg_page1000.reg_setting != NULL) {
		kfree(dbg_reg_page1000.reg_setting);
		dbg_reg_page1000.reg_setting = NULL;
	}
	return val;
}
...
uint32_t cam_sensor_dbg_get_dcg_value_gn3(struct cam_sensor_ctrl_t* s_ctrl)
{
	struct cam_sensor_i2c_reg_setting dbg_reg_page4000 = { 0 };
	struct cam_sensor_i2c_reg_setting dbg_reg_page2000 = { 0 };
	const uint32_t debug_addr_check_dcg = 0x0E84;

	int32_t rc = 0;
	uint32_t val = 0;
	uint32_t size = 0;

	size = ARRAY_SIZE(gn3_page_4000_reg_array);
	dbg_reg_page4000.reg_setting = kmalloc(sizeof(struct cam_sensor_i2c_reg_array) * size, GFP_KERNEL);
	if (dbg_reg_page4000.reg_setting != NULL) {
		dbg_reg_page4000.size = size;
		dbg_reg_page4000.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
		dbg_reg_page4000.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
		dbg_reg_page4000.delay = 0;
		memcpy(dbg_reg_page4000.reg_setting, &gn3_page_4000_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
	}

	size = ARRAY_SIZE(gn3_page_2000_reg_array);
	dbg_reg_page2000.reg_setting = kmalloc(sizeof(struct cam_sensor_i2c_reg_array) * size, GFP_KERNEL);
	if (dbg_reg_page2000.reg_setting != NULL) {
		dbg_reg_page2000.size = size;
		dbg_reg_page2000.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
		dbg_reg_page2000.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
		dbg_reg_page2000.delay = 0;
		memcpy(dbg_reg_page2000.reg_setting, &gn3_page_2000_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
	}

	// page 2000
	rc = camera_io_dev_write(&s_ctrl->io_master_info, &dbg_reg_page2000);
	if (rc < 0)
		CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] Failed to write dbg_write_setting_2000 %d", rc);
	...
	if (dbg_reg_page2000.reg_setting != NULL) {
		kfree(dbg_reg_page2000.reg_setting);
		dbg_reg_page2000.reg_setting = NULL;
	}
	return val;
}
...
void cam_sensor_dbg_print_vc(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	case SENSOR_ID_S5KGN3:
	case SENSOR_ID_S5K3LU:
		vc_pick_idx = e_vc_addr_260_264_1b_read;
		break;
	case SENSOR_ID_S5KJN3:
		vc_pick_idx = e_vc_addr_260_264_2b_read;
		break;
	case SENSOR_ID_IMX564:
	case SENSOR_ID_IMX754:
	...
	if (SENSOR_API_RESULT_AEB_ON == is_aeb_activated)
	{
		...
		if (vc_pick_idx == e_vc_addr_invalid) {
			...
			if (vc_addr_info[vc_pick_idx].pd_addr != 0x0000) {
				rc = camera_io_dev_read(
					&(s_ctrl->io_master_info),
			...
			if ((s_ctrl->sensordata->slave_info.sensor_id == SENSOR_ID_S5KHP2) ||
				(s_ctrl->sensordata->slave_info.sensor_id == SENSOR_ID_S5KJN3)) {
				vc_img = (vc_img & 0xf00) >> 8;
				vc_pd_v = (vc_pd_h & 0xf);
				vc_pd_h = (vc_pd_h & 0xf00) >> 8;
			}

			char vc_info_str[256];
			if (vc_addr_info[vc_pick_idx].pd_addr != 0x0000) {
				scnprintf(vc_info_str, sizeof(vc_info_str), "VC img 0x%x, hpd 0x%x, vpd 0x%x (%dByte)", vc_img, vc_pd_h, vc_pd_v, vc_addr_info[vc_pick_idx].data_sz);
			} else {
				scnprintf(vc_info_str, sizeof(vc_info_str), "VC img 0x%x, (%dByte)", vc_img, vc_addr_info[vc_pick_idx].data_sz);
			}

			if (s_ctrl->sensordata->slave_info.sensor_id == SENSOR_ID_S5KHP2) {
				selected_gain = cam_sensor_dbg_get_dcg_value_hp2(s_ctrl);
				if (selected_gain) {
					scnprintf(vc_info_str + strlen(vc_info_str), sizeof(vc_info_str) - strlen(vc_info_str),
						" %s", selected_gain == 0x000F ? "LCG" : selected_gain == 0x0007 ? "HCG" : "");
				}
			} else if (s_ctrl->sensordata->slave_info.sensor_id == SENSOR_ID_S5KGN3) {
				selected_gain = cam_sensor_dbg_get_dcg_value_gn3(s_ctrl);
				if (selected_gain) {
					scnprintf(vc_info_str + strlen(vc_info_str), sizeof(vc_info_str) - strlen(vc_info_str),
						" %s", selected_gain == 0x0001 ? "LCG" : selected_gain == 0x0101 ? "HCG" : "");
				}
			}


			CAM_INFO(CAM_SENSOR_UTIL, "[AEB_DBG]%s[%s] AEB off, %s reqId %lld",
				s_ctrl->is_bubble_packet == true ? "[BUBBLE]" : "",
				s_ctrl->sensor_name,
				vc_info_str,
	...
	int i2c_debug_cnt)
{
	...
	if (strstr(debug_sensor_name, s_ctrl->sensor_name) != NULL) {
		CAM_INFO(CAM_SENSOR_UTIL, "sensor_name:%s", s_ctrl->sensor_name);
		for (i = 0; i < size; i++) {
			if (i2c_debug_cnt == 0) {
...
void cam_sensor_dbg_regdump_common(struct cam_sensor_ctrl_t* s_ctrl)
{
	uint32_t i;
	...
	int tbl_size = 0;
	const struct st_exposure_reg_dump_addr* p_arr = NULL;

	switch (s_ctrl->sensordata->slave_info.sensor_id)
	{
		case SENSOR_ID_S5KGN3:
			p_arr = gn3_dump_addr_arr;
			tbl_size = sizeof(gn3_dump_addr_arr) / sizeof(struct st_exposure_reg_dump_addr);
			break;
		case SENSOR_ID_S5KJN3:
			p_arr = jn3_dump_addr_arr;
			tbl_size = sizeof(jn3_dump_addr_arr) / sizeof(struct st_exposure_reg_dump_addr);
			break;
		case SENSOR_ID_IMX564:
		case SENSOR_ID_IMX754:
			p_arr = imx564_754_exp_addr_info;
			tbl_size = sizeof(imx564_754_exp_addr_info) / sizeof(struct st_exposure_reg_dump_addr);
			break;
		default:
			break;
	}
...
void cam_sensor_dbg_regdump_hp2(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	struct cam_sensor_i2c_reg_setting dbg_reg_setting_p1002;
	...
	if (dbg_reg_setting_p4000.reg_setting != NULL) {
		...
		memcpy(dbg_reg_setting_p4000.reg_setting, &hp2_page_4000_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
	}

	size = ARRAY_SIZE(hp2_page_1001_reg_array);
	...
	if (dbg_reg_setting_p1001.reg_setting != NULL) {
		...
		memcpy(dbg_reg_setting_p1001.reg_setting, &hp2_page_1001_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
	}
	...
	if (dbg_reg_setting_p1000.reg_setting != NULL) {
		...
		memcpy(dbg_reg_setting_p1000.reg_setting, &hp2_page_1000_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
	}

		size = ARRAY_SIZE(hp2_page_1002_reg_array);
		dbg_reg_setting_p1002.reg_setting = kmalloc(sizeof(struct cam_sensor_i2c_reg_array) * size, GFP_KERNEL);
		if (dbg_reg_setting_p1002.reg_setting != NULL) {
				dbg_reg_setting_p1002.size = size;
				dbg_reg_setting_p1002.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
				dbg_reg_setting_p1002.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
				dbg_reg_setting_p1002.delay = 0;
				memcpy(dbg_reg_setting_p1002.reg_setting, &hp2_page_1002_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
		}


	size = ARRAY_SIZE(hp2_page_1003_reg_array);
	...
	if (dbg_reg_setting_p1003.reg_setting != NULL) {
		...
		memcpy(dbg_reg_setting_p1003.reg_setting, &hp2_page_1003_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
	}
	...
	for (i = 0; i < sizeof(hp2_dump_addr_page4000) / sizeof(uint32_t); i++)
	{
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			hp2_dump_addr_page4000[i],
			&val, CAMERA_SENSOR_I2C_TYPE_WORD,
			CAMERA_SENSOR_I2C_TYPE_WORD, false);
	...
	for (i = 0; i < sizeof(hp2_dump_addr_page1001) / sizeof(uint32_t); i++)
	{
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			hp2_dump_addr_page1001[i],
			&val, CAMERA_SENSOR_I2C_TYPE_WORD,
			CAMERA_SENSOR_I2C_TYPE_WORD, false);
	...
	for (i = 0; i < sizeof(hp2_dump_addr_page1000) / sizeof(uint32_t); i++)
	{
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			hp2_dump_addr_page1000[i],
			&val, CAMERA_SENSOR_I2C_TYPE_WORD,
			CAMERA_SENSOR_I2C_TYPE_WORD, false);
		 ...
		 *  page 1002
		 */
		rc = camera_io_dev_write(&s_ctrl->io_master_info, &dbg_reg_setting_p1002);
		if (rc < 0)
				CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] Failed to write dbg_write_setting_1002 %d", rc);

		for (i = 0; i < sizeof(hp2_dump_addr_page1002) / sizeof(uint32_t); i++)
		{
				rc = camera_io_dev_read(
						&(s_ctrl->io_master_info),
						hp2_dump_addr_page1002[i],
						&val, CAMERA_SENSOR_I2C_TYPE_WORD,
						CAMERA_SENSOR_I2C_TYPE_WORD, false);
				...
				if (rc < 0)
						...
						CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] PAGE_1002 addr:0x%4x  val:0x%4x",
								hp2_dump_addr_page1002[i], val);
				}
		}
	...
	if (dbg_reg_setting_p1002.reg_setting != NULL) {
		kfree(dbg_reg_setting_p1002.reg_setting);
		dbg_reg_setting_p1002.reg_setting = NULL;
	}
	if (dbg_reg_setting_p1003.reg_setting != NULL) {
...
void cam_sensor_dbg_regdump(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	case SENSOR_ID_IMX564:
	case SENSOR_ID_IMX754:
		cam_sensor_dbg_regdump_common(s_ctrl);
		break;
	default:
		CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] not supported 0x%x", s_ctrl->sensordata->slave_info.sensor_id);
		break;
	}
}
...
void cam_sensor_dbg_regdump_stream_on_fail_hp2(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	if (dbg_reg_page1001.reg_setting != NULL) {
		...
		memcpy(dbg_reg_page1001.reg_setting, &hp2_page_1001_reg_array, sizeof(struct cam_sensor_i2c_reg_array) * size);
	}
