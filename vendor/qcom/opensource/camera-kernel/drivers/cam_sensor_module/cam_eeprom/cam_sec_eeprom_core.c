...
#if defined(CONFIG_SAMSUNG_FRONT_AFCAL_M2)
		num_idx = 3;
		af_idx[2].offset = AF_CAL_M2_OFFSET_FROM_AF;
#else
		num_idx = 2;
#endif
	}
	...
	struct cam_eeprom_ctrl_t *e_ctrl, struct config_info_t *config_info)
{
	...
	config_index = ADDR_M_OIS;
	if (e_ctrl->soc_info.index == SEC_TELE_SENSOR ||
		e_ctrl->soc_info.index == SEC_TELE2_SENSOR)
		config_index = ADDR_S_OIS;
	...
	if (is_valid_index(config_info, config_index, &config_addr) == 1) {
		...
		*cross_talk_result = (isCal == 0) ? 1 : 0;
		...
		if (e_ctrl->soc_info.index == SEC_TELE2_SENSOR) {
			config_addr += OIS_5X_VENDOR_ID_OFFSET;
			ois_5x_vendor_id = e_ctrl->cal_data.mapdata[config_addr];
			CAM_DBG(CAM_EEPROM, "config_addr 0x%04X, ois_5x_vendor_id 0x%02X",
				config_addr, ois_5x_vendor_id);
			config_addr -= OIS_5X_VENDOR_ID_OFFSET;
		}
#endif
	}
...
int cam_sec_eeprom_reset_module_info(struct cam_eeprom_ctrl_t *e_ctrl)
{
	...
	if (!gpio_get_value(UPPER_C2C_DET_GPIO) && (e_ctrl->soc_info.index == SEC_FRONT_TOP_SENSOR)) {
		strlcpy(module_id[index], "00000", FROM_MODULE_ID_SIZE);
	} else				
#endif
	{
	...
	struct cam_eeprom_ctrl_t *e_ctrl, struct module_info_t *m_info)
{
	...
	if (version_isp != 'Q' && version_isp != 'U' &&
		version_isp != 'A' && version_isp != 'X' && version_isp != 'E' && version_isp != 'L') {
