...
eeropm_crc_check :
		...
		if (rc && !gpio_get_value(UPPER_C2C_DET_GPIO)) {
#else
		if (rc) {
#endif
			CAM_ERR(CAM_EEPROM,
...
#if defined(CONFIG_SEC_Q7Q_PROJECT) || defined(CONFIG_SEC_Q7MQ_PROJECT)
			if (e_ctrl->soc_info.index == SEC_FRONT_TOP_SENSOR || e_ctrl->soc_info.index == SEC_FRONT_SENSOR) {
				struct cam_eeprom_memory_block_t *block = &e_ctrl->cal_data;

				if (soc_private->i2c_info.slave_addr == 0xA2) {
					soc_private->i2c_info.slave_addr = 0xA0;
				} else if (soc_private->i2c_info.slave_addr == 0xA0) {
					soc_private->i2c_info.slave_addr = 0xA2;
				}

				CAM_DBG(CAM_EEPROM, "slave_addr = 0x%X",soc_private->i2c_info.slave_addr);

				for (i = 0; i < block->num_map; i++) {
					block->map[i].saddr = soc_private->i2c_info.slave_addr;
				}
...
#if defined(CONFIG_CAMERA_SYSFS_V2)
		memset(&module_info, 0, sizeof(struct module_info_t));
		memset(config_info, 0, sizeof(config_info));
		if (1 < e_ctrl->cal_data.num_map) {
			if (crc_check_retry_cnt == 0) {
				rc = cam_sec_eeprom_get_custom_info(
					e_ctrl, csl_packet, config_info);
				if (rc < 0) {
					CAM_INFO(CAM_EEPROM, "cam_sec_eeprom_get_custom_info failed");
				}
			}

			e_ctrl->is_supported |= cam_sec_eeprom_match_crc(&e_ctrl->cal_data,
				e_ctrl->soc_info.index, config_info);
...
#if defined(CONFIG_SEC_Q7Q_PROJECT) || defined(CONFIG_SEC_Q7MQ_PROJECT)
				if ((crc_check_retry_cnt < 10) && !gpio_get_value(UPPER_C2C_DET_GPIO)) {
#else
			...
			rc = cam_sec_eeprom_update_module_info(e_ctrl, &module_info, config_info);
			if (rc < 0) {
