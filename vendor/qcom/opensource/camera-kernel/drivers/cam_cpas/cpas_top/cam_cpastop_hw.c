...
uint32_t g_qchannel_status_rt = 0;
uint32_t g_qchannel_status_nrt = 0;
	...
	bool power_on, bool force)
{
	...
	for (i = 0; i < cpas_info->num_qchannel; i++) {
		...
		qchannel_status = cam_io_r_mb(soc_info->reg_map[reg_indx].mem_base +
			qchannel_info->qchannel_status);
		if (i==CAM_CAMNOC_HW_RT)
			g_qchannel_status_rt = qchannel_status;
		else if (i==CAM_CAMNOC_HW_NRT)
			g_qchannel_status_nrt = qchannel_status;
