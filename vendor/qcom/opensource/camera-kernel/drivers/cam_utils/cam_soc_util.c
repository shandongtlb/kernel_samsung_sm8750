...
int cam_soc_util_regulator_set_mode(
	struct regulator *rgltr,
	const char *rgltr_name,
	unsigned int mode)
{
	int rc = 0, enable_cnt = 0;

	enable_cnt = cam_wrapper_regulator_is_enabled(rgltr, rgltr_name);

	if (((mode == REGULATOR_MODE_FAST) && (enable_cnt == 0)) ||
		((mode == REGULATOR_MODE_NORMAL) && (enable_cnt == 0))) {
		rc = regulator_set_mode(rgltr, mode);
		if (rc < 0) {
			if (rc == -EINVAL)
				CAM_DBG(CAM_UTIL, "%s Not support regulator_set_mode",
					rgltr_name);
			else
				CAM_ERR(CAM_UTIL, "%s regulator_set_mode failed, %d",
					rgltr_name, rc);
		}
		else
		{
			CAM_DBG(CAM_UTIL, "%s support regulator_set_mode success",
						rgltr_name);
		}
	}
