	...
	uint64_t request_id, struct cam_isp_prepare_hw_update_data *prepare_hw_data)
{
	...
	if (csid_update_valid) {
		if (!ctx->flags.is_aeb_mode) {
			CAM_ERR(CAM_ISP, "Exp order update for csid not supported for non-aeb cases");
			return -EPERM;
        	}
	...
	struct cam_isp_in_port_generic_info     *in_port)
{
	...
	ife_ctx->num_acq_vfe_out++;
	ife_ctx->ife_acquire_wm_mask |= BIT_ULL(out_port->res_type - CAM_ISP_IFE_OUT_RES_BASE);
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			ife_ctx->ife_acquire_wm_mask |=
				BIT_ULL(out_port->res_type - CAM_ISP_IFE_OUT_RES_BASE);
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		sfe_src_res->num_children++;
		ife_ctx->num_acq_sfe_out++;
		ife_ctx->sfe_acquire_wm_mask |=
			BIT_ULL(sfe_out_res->res_id - CAM_ISP_SFE_OUT_RES_BASE);
	}
...
#if defined(CONFIG_SAMSUNG_CAMERA)
		if (hw_intf)
			CAM_INFO(CAM_ISP, "[AEB_DBG] CSID[%u] MUP %u reqId %lld",
	...
	void *prepare_hw_update_args)
{
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		if (ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE) {
			...
			io_buf_info.prepare->ife_acquire_wm_mask = ctx->ife_acquire_wm_mask;
			...
			io_buf_info.prepare->sfe_acquire_wm_mask = ctx->sfe_acquire_wm_mask;
			rc = cam_isp_add_io_buffers(&io_buf_info);
		}
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	case CAM_HW_MGR_CMD_INTERNAL:
		...
		case CAM_ISP_HW_MGR_RESET_OUT_OF_SYNC_CNT:
			...
			ctx->try_recovery_cnt = 0;
			break;
		case CAM_ISP_HW_MGR_DETECT_CDM_HANG: {
			int rc = 0;
			rc = cam_cdm_detect_hang_error(ctx->cdm_handle);
			if (rc < 0)
				cam_cdm_dump_debug_registers(ctx->cdm_handle);
