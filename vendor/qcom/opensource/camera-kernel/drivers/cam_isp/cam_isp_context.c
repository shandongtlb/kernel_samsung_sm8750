...
static void __cam_isp_ctx_dump_recent_req_record(struct cam_isp_context *ctx_isp);
...
static void __cam_isp_ctx_update_recent_req_record(
	struct cam_isp_context *ctx_isp,
	struct cam_isp_ctx_req *req_isp)
{
	...
	if (!req_isp) {
		CAM_ERR(CAM_ISP, "Invalid arg for event");
		return;
	}
	if (!ctx_isp) {
	...
	INC_HEAD(&ctx_isp->dbg_monitors.recent_req_head,
		CAM_ISP_CTX_MAX_FRAME_RECORDS, &iterator);
	CAM_DBG(CAM_ISP, "ctx: %d Added Req %lld in recent req queue index %d req_head: %d",
			ctx->ctx_id, req_isp->base->request_id, iterator, atomic64_read(&ctx_isp->dbg_monitors.recent_req_head));
	ctx_isp->dbg_monitors.recent_reqs[iterator] = req_isp;
}
...
static int __cam_isp_ctx_print_event_record(struct cam_isp_context *ctx_isp)
{
	...
	__cam_isp_ctx_dump_recent_req_record(ctx_isp);
...
static void __cam_isp_ctx_dump_recent_req_record(
	struct cam_isp_context *ctx_isp)
{
	int64_t state_head;
	uint32_t index, num_entries, oldest_entry;
	...
	struct cam_isp_ctx_req *req_isp;
	struct cam_context *ctx = ctx_isp->base;
	uint64_t en_mask, done_mask;
	int set_lsb_index;

	state_head = atomic64_read(&ctx_isp->dbg_monitors.recent_req_head);
	CAM_INFO(CAM_ISP, "state head %lld", state_head);
	if (state_head == -1)
		return;

	//state_head = CAM_ISP_CTX_MAX_FRAME_RECORDS;
	if (state_head < CAM_ISP_CTX_MAX_FRAME_RECORDS) {
		num_entries = state_head+1;
		oldest_entry = 0;
	} else {
	...
	index = oldest_entry;
	for (i = 0; i < num_entries; i++) {
		req_isp = ctx_isp->dbg_monitors.recent_reqs[index];
		if (!req_isp) {
			CAM_ERR(CAM_ISP, "NO req at index: %d", i);
			continue;
		}

		if (req_isp->out_res_data.ife_acquire_wm_mask)
			CAM_INFO(CAM_ISP,
				"i %d ctx %d Req:%llu IFE: acq: 0x%llx config: 0x%llx done: 0x%llx",
				index, ctx->ctx_id, req_isp->base->request_id,
				req_isp->out_res_data.ife_acquire_wm_mask,
				req_isp->out_res_data.ife_en_out_res_mask,
				req_isp->out_res_data.ife_done_out_res_mask);

		en_mask = req_isp->out_res_data.ife_en_out_res_mask;
		done_mask = req_isp->out_res_data.ife_done_out_res_mask;

		while(en_mask) {
			set_lsb_index = __ffs(en_mask);
			en_mask &= en_mask - 1;
			is_done = (done_mask >> set_lsb_index) & 0x1;
			if (is_done)
				CAM_INFO(CAM_ISP, "bit %d %s is_done Last Consumed Address 0x%llx",
						set_lsb_index,
						__cam_isp_ife_sfe_resource_handle_id_to_type(
						set_lsb_index + CAM_ISP_IFE_OUT_RES_BASE),
						req_isp->out_res_data.ife_last_consumed_addr[set_lsb_index]);
		}

		if (req_isp->out_res_data.sfe_acquire_wm_mask)
			CAM_INFO(CAM_ISP,
				"i %d ctx %d Req:%llu SFE: acq: 0x%llx config = 0x%llx done= 0x%llx",
				index, ctx->ctx_id, req_isp->base->request_id,
				req_isp->out_res_data.sfe_acquire_wm_mask,
				req_isp->out_res_data.sfe_en_out_res_mask,
				req_isp->out_res_data.sfe_done_out_res_mask);
		index = (index + 1) % CAM_ISP_CTX_MAX_FRAME_RECORDS;
	}
}
	...
	struct cam_context *ctx, struct cam_ctx_request *req)
{
	...
	if (req_isp_old->hw_update_data.packet_opcode_type ==
		...
		if (!rc) {
			...
			CAM_DBG(CAM_ISP,"KMD Patch - Before Copy data to old request for EPCR");
			...
			CAM_DBG(CAM_ISP,"KMD Patch - After Copy data to old request for EPCR");
	...
	struct cam_isp_ctx_req *req_isp)
{
	...
	if ((ctx_isp->congestion_cnt >= CAM_ISP_CONTEXT_CONGESTION_CNT_MAX) &&
		...
		if (isp_ctx_debug.dump_last_req_buf_done_fail)
			__cam_isp_ctx_dump_recent_req_record(ctx_isp);
	}
}

static void __cam_isp_ctx_buf_done_fail_log_status(
	struct cam_isp_context *ctx_isp, uint64_t request_id,
	struct cam_isp_ctx_req *req_isp)
{
	int i;
	...
	struct cam_ctx_request *req)
{
	...
	__cam_isp_ctx_update_recent_req_record(ctx_isp, req_isp);
	return rc;
}
	...
	struct cam_isp_hw_done_event_data *done_next_req)
{
	...
	for (i = 0; i < comp_grp->num_res; i++) {
		...
		if (!req_isp->bubble_detected) {
			...
			if (done->hw_type != CAM_ISP_HW_TYPE_SFE) {
				res_index = req_isp->fence_map_out[j].resource_handle -
					CAM_ISP_IFE_OUT_RES_BASE;
				if (res_index <= CAM_ISP_IFE_OUT_RES_STATS_AEC_BHIST) {
					req_isp->out_res_data.ife_done_out_res_mask |=
						BIT_ULL(res_index);
					req_isp->out_res_data.ife_last_consumed_addr[res_index] = done->last_consumed_addr;
				}
			}
...
check_deferred:
	...
	if (dump_req) {
		__cam_isp_ctx_dump_recent_req_record(ctx_isp);
		__cam_isp_ctx_buf_done_fail_log_status(ctx_isp,
...
hw_dump:
	...
	hw_cmd_args.ctxt_to_hw_map = ctx->ctxt_to_hw_map;
	hw_cmd_args.cmd_type = CAM_HW_MGR_CMD_INTERNAL;
	isp_hw_cmd_args.cmd_type = CAM_ISP_HW_MGR_DETECT_CDM_HANG;
	isp_hw_cmd_args.cmd_data = &dump_args;
	hw_cmd_args.u.internal_args = (void *)&isp_hw_cmd_args;
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"cdm hang detect failed %lld, rc: %d, ctx: %u, link: 0x%x",
			req->request_id, rc, ctx->ctx_id, ctx->link_hdl);
	}
	...
	void *cmd)
{
	...
	atomic64_set(&ctx_isp->dbg_monitors.recent_req_head, -1);
	...
	struct cam_context *ctx, struct cam_config_dev_cmd *cmd)
{
	...
	req_isp->out_res_data.ife_en_out_res_mask = 0;
	req_isp->out_res_data.sfe_en_out_res_mask = 0;
	req_isp->out_res_data.ife_done_out_res_mask = 0;
	req_isp->out_res_data.sfe_done_out_res_mask = 0;
	...
	req->pf_data.req = req;
	req->packet = packet;
	req_isp->out_res_data.ife_en_out_res_mask = cfg.ife_en_out_res_mask;
	req_isp->out_res_data.ife_en_out_res_mask = cfg.ife_en_out_res_mask;
	req_isp->out_res_data.ife_acquire_wm_mask = cfg.ife_acquire_wm_mask;
	req_isp->out_res_data.sfe_acquire_wm_mask = cfg.sfe_acquire_wm_mask;
	...
	CAM_DBG(CAM_ISP,
		...
		ctx->ctx_id, ctx->link_hdl);
	CAM_DBG(CAM_ISP,
		"ife_en_out_res_mask 0x%llx sfe_en_out_res_mask 0x%llx",
		req_isp->out_res_data.ife_en_out_res_mask,
		req_isp->out_res_data.sfe_en_out_res_mask);
	...
	struct cam_config_dev_cmd *cmd)
{
	...
	len = cam_context_parse_config_cmd(ctx, cmd, &packet);

	packet_opcode = ((packet->header.op_code) + 1) & 0xF;
	CAM_INFO(CAM_ISP, "pkt opcode = %u", packet_opcode);
	if ((!ctx_isp->init_received) || (packet_opcode == CAM_ISP_PACKET_UPDATE_DEV)) {
		CAM_WARN(CAM_ISP,
	...
	if (ctx_isp->standby_en) {
		...
		if (ctx->ctx_crm_intf && ctx->ctx_crm_intf->notify_msg) {
			...
			cam_common_mem_free(packet);
			CAM_INFO(CAM_ISP, "ISP SYNCED_RESUME");
			return rc;
		}
	}
...
static int cam_isp_context_debug_register(void)
{
	...
	debugfs_create_u32("dump_last_req_buf_done_fail", 0644,
		isp_ctx_debug.dentry, &isp_ctx_debug.dump_last_req_buf_done_fail);

	isp_ctx_debug.wait_indefinitely_mup_sync = true;
	return 0;
}
	...
	int img_iommu_hdl)
{
	...
	atomic64_set(&ctx->dbg_monitors.recent_req_head, -1);
