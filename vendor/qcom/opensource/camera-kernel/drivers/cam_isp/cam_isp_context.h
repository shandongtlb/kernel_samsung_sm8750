...
#define CAM_ISP_CTX_RECENT_REQ_MAX_ENTRIES    8
...
#define CAM_ISP_IFE_OUT_RES_MAX 45

struct out_res_data {
	uint64_t ife_acquire_wm_mask;
	uint64_t ife_en_out_res_mask;
	uint64_t ife_done_out_res_mask;
	uint64_t ife_last_consumed_addr[CAM_ISP_IFE_OUT_RES_MAX];
	uint64_t sfe_acquire_wm_mask;
	uint64_t sfe_en_out_res_mask;
	uint64_t sfe_done_out_res_mask;
};
...
struct cam_isp_ctx_debug {
	...
	uint32_t        dump_last_req_buf_done_fail;
};
...
struct cam_isp_ctx_req {
	...
	struct out_res_data                   out_res_data;
};
...
struct cam_isp_context_debug_monitors {
	...
	struct cam_isp_ctx_req               *recent_reqs[
		CAM_ISP_CTX_RECENT_REQ_MAX_ENTRIES];
	atomic64_t                            recent_req_head;
	/* Req event monitor */
