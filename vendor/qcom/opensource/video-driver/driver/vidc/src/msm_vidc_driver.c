...
int msm_vidc_init_instance_caps(struct msm_vidc_core *core)
{
	...
	for (i = 0; i < codecs_count; i++) {
		core->inst_caps[i].cap = devm_kzalloc(&core->pdev->dev,
			(INST_CAP_MAX + 1) * sizeof(struct msm_vidc_inst_cap), GFP_KERNEL);
		if (!core->inst_caps[i].cap) {
			d_vpr_e("%s: failed to alloc memory for instance cap %d\n", __func__, i);
			rc = -ENOMEM;
			goto error;
		}
	}
