...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
#define UPDATE_BUSY_VAL_TRACE		8000
#endif
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
void kgsl_pwrctrl_trace_busy_time(struct kgsl_device *device, u64 time, u64 busy)
{
	struct kgsl_clk_stats *stats = &device->pwrctrl.clk_stats;
	struct kgsl_pwrctrl *pwr = &device->pwrctrl;
	u32 max_pwrlevel = max_t(u32, READ_ONCE(pwr->thermal_pwrlevel), READ_ONCE(pwr->pmqos_max_pwrlevel));
	unsigned int busy_percent = 0;
    
	stats->trace_total += time;
	stats->trace_busy += busy;

	if (stats->trace_total < UPDATE_BUSY_VAL_TRACE)
		return;

	/* Update the output regularly and reset the counters. */
	stats->trace_total_old = stats->trace_total;
	stats->trace_busy_old = stats->trace_busy;
	stats->trace_total = 0;
	stats->trace_busy = 0;
	
	if (stats->trace_total_old != 0)
		busy_percent = (stats->trace_busy_old * 100) / stats->trace_total_old;
	
	trace_clock_set_rate("Gpu Utilization", busy_percent, 0);
	trace_clock_set_rate("Gpu Min Limit", pwr->pwrlevels[pwr->min_pwrlevel].gpu_freq, 0);
	trace_clock_set_rate("Gpu Max Limit", pwr->pwrlevels[max_pwrlevel].gpu_freq, 0);
}
#endif
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)	
	kgsl_pwrctrl_trace_busy_time(device, time, busy);
#endif
				...
				unsigned int state)
{
	...
	/* Reset the stats if GPU is OFF */
	if (!test_bit(KGSL_PWRFLAGS_AXI_ON, &device->pwrctrl.power_flags)) {
		trace_clock_set_rate("Gpu Utilization", 0, 0);
	}
#endif
}
