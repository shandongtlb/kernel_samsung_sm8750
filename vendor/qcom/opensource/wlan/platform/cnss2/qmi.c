...
static int cnss_ini_read_values(char **main_cursor,
	...
	while (*cursor != '\0') {
		...
		if (value) {
			...
			*main_cursor = cursor;
			...
			if (key[len - 1] != ']') {
				cnss_pr_err("Invalid *.ini syntax '%s'", key);
				return -EINVAL;
			} else {
				key[len - 1] = '\0';
				*read_key = key + 1;
...
static int cnss_validate_key(char *key)
{
	int i;
...
static int cnss_validate_value(char *value)
{
	int i;
...
static bool
cnss_check_for_valid_characters(const struct firmware *fw)
{
	char *cursor;
	char *read_key;
	char *read_value;
	...
	while (*cursor != '\0') {
		...
		default:
			if (!isalnum(*val)) {
				cnss_pr_err("Found invalid character %c", *cursor);
				ret = false;
				goto out;
			}
			cursor++;
	...
	cursor = buf;
	while ((status = cnss_ini_read_values(&cursor, &read_key, &read_value,
					      &section_item)) == 0) {
		if (!section_item) {
			cnss_pr_dbg("Key = %s: Value = %s", read_key, read_value);
			status = cnss_validate_key(read_key);
			if (!status) {
				cnss_pr_dbg("Invalid Key = %s", read_key);
				ret = false;
				goto out;
			}
			status = cnss_validate_value(read_value);
			if (!status) {
				cnss_pr_dbg("Invalid Value = %s", read_value);
				ret = false;
				goto out;
			}
		}
				 ...
				 enum wlfw_ini_file_type_v01 file_type)
{
	...
	if (ret || !cnss_check_for_valid_characters(fw)) {
		if (ret)
