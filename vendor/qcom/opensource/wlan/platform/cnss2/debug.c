...
#ifdef SEC_FACTORY_BUILD
		/* CN 07584019 - Added to Change Gen switch in FTM.
			it neeed to be used along with pci_configs=1
		*/
		} else if (sysfs_streq(cmd, "pcie_gen_1")) {
			cnss_pr_info("Force set QMI_PCIE_GEN_SPEED_1_V01\n");
			cnss_set_pcie_gen(pci_priv, QMI_PCIE_GEN_SPEED_1_V01);
		} else if (sysfs_streq(cmd, "pcie_gen_2")) {
			cnss_pr_info("Force set QMI_PCIE_GEN_SPEED_2_V01\n");
			cnss_set_pcie_gen(pci_priv, QMI_PCIE_GEN_SPEED_2_V01);
		} else if (sysfs_streq(cmd, "pcie_gen_3")) {
			cnss_pr_info("Force set QMI_PCIE_GEN_SPEED_3_V01\n");
			cnss_set_pcie_gen(pci_priv, QMI_PCIE_GEN_SPEED_3_V01);
#endif
		} else {
