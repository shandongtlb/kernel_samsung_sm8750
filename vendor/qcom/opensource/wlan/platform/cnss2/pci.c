...
int cnss_set_pcie_gen(struct cnss_pci_data *pci_priv,
			     unsigned int gen_speed)
{
	struct mhi_link_info link_info;

	link_info.target_link_speed = gen_speed;
	link_info.target_link_width = pci_priv->def_link_width;

	return cnss_mhi_bw_scale(pci_priv->mhi_ctrl, &link_info);
}
#endif
