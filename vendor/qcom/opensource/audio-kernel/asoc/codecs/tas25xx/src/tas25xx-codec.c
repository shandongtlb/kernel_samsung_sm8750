	...
	struct snd_soc_dai *dai)
{
	...
	for (i = 0; i < p_tas25xx->ch_count; i++) {
		...
		if (!ret) {
			/* reset ch_ignore when I2C transaction succeeds  */
			if (is_error_on_ch(p_tas25xx->amp_i2c_err, i))
				reset_ch_ignore(p_tas25xx->amp_i2c_err, i);

			if (status > 0)
				dev_info(plat_data->dev, "reg are found to be in default, reinit port setting\n");
		} else {
