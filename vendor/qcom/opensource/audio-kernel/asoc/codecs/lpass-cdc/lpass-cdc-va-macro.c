	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (ret) {
			dev_err_ratelimited(va_dev, "%s: Setting voltage failed, err = %d\n",
				__func__, ret);
			sdp_info_print("%s: Setting voltage failed, err = %d\n",
				__func__, ret);
			return ret;
		}
		...
		if (ret) {
			dev_err_ratelimited(va_dev, "%s: Setting current failed, err = %d\n",
				__func__, ret);
			sdp_info_print("%s: Setting current failed, err = %d\n",
				__func__, ret);
			return ret;
		}
		...
		if (ret) {
			dev_err_ratelimited(va_dev, "%s: regulator enable failed, err = %d\n",
				__func__, ret);
			return ret;
			sdp_info_print("%s: regulator enable failed, err = %d\n",
				__func__, ret);
		} else {
#if defined(CONFIG_SEC_AUDIO_DEBUG)
			sec_audio_debug_codec_micbias_enable(true);
#endif
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		...
		if (ret) {
			dev_err_ratelimited(va_dev, "%s: regulator disable failed, err = %d\n",
				__func__, ret);
			sdp_info_print("%s: regulator disable failed, err = %d\n",
				__func__, ret);
			return ret;
		}
...
#if defined(CONFIG_SEC_AUDIO_DEBUG)
			sec_audio_debug_codec_micbias_enable(false);
#endif
		break;
	}
	return 0;
