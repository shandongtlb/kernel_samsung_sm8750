...
#define AMP_DRIVER_NAME "tas25xx"
#endif
#endif
...
static struct regulator *buckboost_supply;
...
static int msm_amp_init(struct snd_soc_pcm_runtime *rtd)
{
#if defined(CONFIG_SEC_AUDIO_DEBUG)
	int ret;
	struct snd_soc_component *amp_component = NULL;
#endif

	pr_info("%s\n", __func__);
	register_amp_callback();

#if defined(CONFIG_SEC_AUDIO_DEBUG)
	amp_component = snd_soc_rtdcom_lookup(rtd, AMP_DRIVER_NAME);
	if (!amp_component) {
		pr_err("%s: could not find component for amp\n",
			__func__);
		return 0;
	}

	ret = sec_audio_debug_get_rx_component(amp_component);
	if (ret < 0) {
		pr_err("%s: could not get drvdata for amp\n",
			__func__);
	}
#endif
...
#if defined(CONFIG_SEC_AUDIO_DEBUG)
static struct snd_soc_dai_link sec_audio_debug_dai_links[] = {
	{
		.name = "SEC_AUDIO_DEBUG",
		.stream_name = "SEC_AUDIO_DEBUG",
		.no_pcm = 1,
		.dpcm_capture = 1,
		.ignore_pmdown_time = 1,
		.ignore_suspend = 1,
		SND_SOC_DAILINK_REG(sec_audio_debug),
	},
};
#endif
...
#if defined(CONFIG_SEC_AUDIO_DEBUG)
		rc = of_property_read_u32(dev->of_node,
				"qcom,sec_audio_debug", &val);
		if (!rc && val) {
			memcpy(msm_sun_dai_links + total_links,
					sec_audio_debug_dai_links,
					sizeof(sec_audio_debug_dai_links));
			total_links += ARRAY_SIZE(sec_audio_debug_dai_links);
		}
#endif
...
static int buckboost_ctl_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = regulator_get_mode(buckboost_supply);
	if (ret == REGULATOR_MODE_INVALID)
		pr_err("%s: Failed to get regulator mode\n", __func__);
	else
		pr_info("%s: Current buckboost mode: %d\n", __func__, ret);
...
static int buckboost_ctl_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	int val = ucontrol->value.enumerated.item[0];

	pr_info("%s: Set buckboost to %d\n", __func__, val);

	if (!buckboost_supply)
		return -EINVAL;

	ret = regulator_set_mode(buckboost_supply, val);
	if (ret < 0)
		pr_err("%s: Failed to set buckboost mode: %d\n", __func__, ret);

	return 0;
}

static const char *const buckboost_text[] = {"ZERO", "FPWM", "SKIP"}; /* Buckboost mode*/
static SOC_ENUM_SINGLE_EXT_DECL(buckboost_mode, buckboost_text);

static const struct snd_kcontrol_new buckboost_mixer_controls[] = {
	SOC_ENUM_EXT("Buckboost mode", buckboost_mode,
			buckboost_ctl_get, buckboost_ctl_put),
};
...
static int msm_rx_tx_codec_init(struct snd_soc_pcm_runtime *rtd)
{
...
#if defined(CONFIG_SEC_AUDIO_DEBUG)
	ret = sec_audio_debug_get_tx_component(lpass_cdc_component);
	if (ret < 0) {
		pr_err("%s: could not get drvdata for lpass-cdc\n",
			__func__);
		ret = 0;
	}
#endif
	...
	ret = snd_soc_add_component_controls(lpass_cdc_component,
			buckboost_mixer_controls, ARRAY_SIZE(buckboost_mixer_controls));
	if (ret < 0) {
		sdp_boot_print("%s: add buckboost controls failed: %d\n",
				__func__, ret);
		pr_err("%s: add buckboost controls failed: %d\n",
				__func__, ret);
		ret = 0;
	}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node,
						"qcom,max_defer_count",
						&max_defer_count);
	if (ret) {
		dev_info(&pdev->dev,
		"%s: max_defer_count property missing in DT %s, ret = %d\n",
		__func__, pdev->dev.of_node->full_name, ret);
		max_defer_count = 10;
	}

	check_snd_component(card, max_defer_count);
	...
	if (of_parse_phandle(pdev->dev.of_node, "buckboost-supply", 0)) {
		buckboost_supply = devm_regulator_get(&pdev->dev, "buckboost");
		if (IS_ERR(buckboost_supply)) {
			ret = PTR_ERR(buckboost_supply);
			dev_err(&pdev->dev,
				"%s: failed to get buckboost-supply: %d\n",
				__func__, ret);
			ret = 0;
		} else
			sdp_boot_print("%s: This board uses buckboost-supply\n", __func__);
