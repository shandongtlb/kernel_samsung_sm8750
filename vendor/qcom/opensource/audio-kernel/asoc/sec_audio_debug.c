...
int sec_audio_debug_tx_status_check(void)
{
	...
	if (tx_debug_info.config_status == NORMAL_STATUS) {
		for(i = 0; i < tx_debug_info.num_mic; i++) {
			if ((dmic_ctl[i] == DMIC_CLK_3_2_MZ)
				|| (dmic_ctl[i] == DMIC_CLK_2_4_MZ)
				|| (dmic_ctl[i] == DMIC_CLK_0_6_MZ))
...
int sec_audio_debug_rx_config_info(int amp_index)
{
	if (!rx_debug_info) {
		pr_err("%s: rx_debug_info is NULL\n", __func__);
		return -EINVAL;
	}
	sec_audio_debug_rx_status_check(rx_debug_info, amp_index);
	return 0;
}
EXPORT_SYMBOL(sec_audio_debug_rx_config_info);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (boot_complete == 2) {
		for (i = 0; i < rx_debug_info->num_amp; i++)
			sec_audio_debug_rx_status_check(rx_debug_info, i);
...
static int sec_audio_debug_probe(struct platform_device *pdev)
{
	...
	if((tx_debug_info.num_mic == 0) || (rx_debug_info->num_amp == 0))
		return EINVAL;
