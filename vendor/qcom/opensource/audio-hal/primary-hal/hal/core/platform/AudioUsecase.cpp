...
Usecase getUsecaseTag(const ::aidl::android::media::audio::common::AudioPortConfig& mixPortConfig) {
    ...
    constexpr auto subDeviceFlags =
            static_cast<int32_t>(1 << flagCastToint(AudioOutputFlags::SUBDEVICE));
#endif
...
#ifdef SEC_AUDIO_MULTI_DEVICE_SOUND_BLE
        } else if (outFlags == subDeviceFlags) {
            tag = Usecase::SUBDEVICE;
#endif
...
#ifdef SEC_AUDIO_MULTI_DEVICE_SOUND_BLE
// [SubdevicePlayback Start]
size_t SubdevicePlayback::getFrameCount(const AudioPortConfig& mixPortConfig) {
    return kPeriodDurationMs * getSampleRate(mixPortConfig).value() / 1000;
}

// [SubdevicePlayback End]
#endif
