    ...
    for (auto& device : devices) {
        ...
        */
        if (palDevices[i].id > PAL_DEVICE_IN_MIN) {
            ...
            else if (palDevices[i].id == PAL_DEVICE_IN_BLUETOOTH_BLE) {
                if (!isBleNrecOn()) {
                    ck_id = CUSTOM_KEY_BT_HEADSET_NREC;
                }
            }
        ...
        */
        ...
        /* Case 4: voice recognition */
            ...
            if (register_voice_keyword) {
                ...
                if (mConcurrentCapture)
                    palDevices[i].id = PAL_DEVICE_IN_HANDSET_VA_MIC;
                else
        ...
        /* Case 10: karaoke */
            ...
            if (getChannelCount(mixPortConfig.channelMask.value()) == 2)
                ck_id = CUSTOM_KEY_PERFORM_DUAL;
            else
                ck_id = CUSTOM_KEY_PERFORM;
        }
...
bool Platform::getBLECaptureEnable()
{
    bool isBLECaptureSupported = true;

    auto deviceCapability = std::make_unique<pal_param_device_capability_t>();
    if (!deviceCapability) {
        LOG(ERROR) << __func__ << ": allocation failed, return capture supported";
        return isBLECaptureSupported;
    }
    ...
    size_t payloadSize = 0;
    deviceCapability->addr.card_id = 0;
    deviceCapability->addr.device_num = 0;
    deviceCapability->config = dynamicMediaConfig.get();
    deviceCapability->id = PAL_DEVICE_OUT_BLUETOOTH_BLE;
    ...
    isBLECaptureSupported = dynamicMediaConfig->jack_status;
...
#ifdef SEC_AUDIO_MULTI_DEVICE_SOUND_BLE
    mUsecaseOpMap[Usecase::SUBDEVICE] = makeUsecaseOps<SubdevicePlayback>();
#endif
...
int Platform::match_device_enums(const AudioDevice& device) const noexcept {
    ...
    } else if (device.type.type == AudioDeviceType::IN_MICROPHONE_BACK) { // AUDIO_DEVICE_IN_BACK_MIC
        ret = AUDIO_DEVICE_IN_BACK_MIC;
...
#ifdef SEC_AUDIO_SOUND_TRIGGER_TYPE
bool Platform::getConcurrentCapture() const noexcept {
    bool status = false;
    bool* payLoad = &status;
    size_t payLoadSize = 0;
    if (int32_t ret =
                ::pal_get_param(PAL_PARAM_ID_GET_SOUND_TRIGGER_CONCURRENT_CAPTURE,
                                reinterpret_cast<void**>(&payLoad), &payLoadSize, nullptr);
        ret) {
        LOG(ERROR) << ": failed to get PAL_PARAM_ID_GET_SOUND_TRIGGER_CONCURRENT_CAPTURE status";
        return false;
    }
    return status;
}
...
#ifdef SEC_AUDIO_SOUND_TRIGGER_TYPE
    mConcurrentCapture = getConcurrentCapture();
#endif
}
