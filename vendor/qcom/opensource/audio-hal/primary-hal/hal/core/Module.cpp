...
ndk::ScopedAStatus Module::updateStreamsConnectedState(const AudioPatch& oldPatch,
    ...
    std::for_each(newConnections.begin(), newConnections.end(), [&](const auto& connectionPair) {
        ...
        if (auto it = oldConnections.find(mixPortConfigId);
                ...
                && (voipRxPortConfigId == mixPortConfigId) 
                && (pf.getCallMode() == AUDIO_MODE_NORMAL)) {
            LOG(DEBUG) << "updateStreamsConnectedState : do route for stream port config id " 
                << mixPortConfigId << " (voipRx)";
            do_route = true;
        }
#endif
        if (auto it = oldConnections.find(mixPortConfigId);
...
ndk::ScopedAStatus Module::connectExternalDevice(const AudioPort& in_templateIdAndAdditionalData,
    ...
    if (connectedPort.profiles.empty()) {
        ...
        auto& connectedDevicePortType = connectedPort.ext.get<AudioPortExt::Tag::device>().device.type;
        if (connectedDevicePortType.type >= AudioDeviceType::OUT_DEFAULT &&
                connectedDevicePortType.connection == AudioDeviceDescription::CONNECTION_USB) {
            onExternalDeviceConnectionChanged(connectedPort, false /*disconnected*/);
            LOG(ERROR) << "To make same USB connection state on AHAL and PAL, "
                       << "send disconnect requests of "
                       << connectedPort.toString();
        }
#endif
        return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_STATE);
...
#ifdef SEC_AUDIO_CALL_VOIP
    if (hasOutputVoipRxFlag(port->flags)) {
        voipRxPortConfigId = in_args.portConfigId;
    }
#endif
