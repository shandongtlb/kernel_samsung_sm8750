...
StreamOutPrimary::StreamOutPrimary(StreamContext&& context, const SourceMetadata& sourceMetadata,
    ...
    } else if (mTag == Usecase::SUBDEVICE) {
        mExt.emplace<SubdevicePlayback>();
#endif
    }
...
bool StreamOutPrimary::isSoundBoosterFoldDegreeSupported() {
    switch (mTag) {
#ifdef SEC_AUDIO_SUPPORT_SOUNDBOOSTER_FOLD_PARAM
        case Usecase::COMPRESS_OFFLOAD_PLAYBACK:
...
#endif
#ifdef SEC_AUDIO_SUPPORT_SOUNDBOOSTER_FOLD_PARAM_ON_DSP
    if (isSoundBoosterFoldDegreeSupported() 
            && isDeviceAvailable(PAL_DEVICE_OUT_SPEAKER)) {
        mAudioEffect.updateSoundBoosterFoldDegree(true /*force_set*/);
    }
#endif
...
#ifdef SEC_AUDIO_COMMON
            if (SecParameters::kCreateMmapBuffer == param.id) {
                // P250404-02485
                // System checks if hal supports updating buffer or not with setParameter.
                // If hal doesn't support it, It should returns not ok.
                return ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
            }
#endif
            continue;
...
#ifdef SEC_AUDIO_PREVOLUME_SOUNDBOOSTER
    // TODO: mPlatform.isSecAudioFeatureSupported() should be removed. Because it was added temporarily to fix the failure of VTS.
    if (mPlatform.isSecAudioFeatureSupported() && mTag == Usecase::VOIP_PLAYBACK) {
        auto streamOut = SecModulePrimary::GetStreamOut(Usecase::MMAP_PLAYBACK);
        if (streamOut && streamOut->HasPalStreamHandle() && hasSpeakerDevice(mConnectedDevices)) {
            LOG(DEBUG) << __func__ << mLogPrefix << ": update sb for mmap";
            mAudioEffect.send_soundbooster_on(PARAM_VOLUME_MMAP, true);
            streamOut->updateSecVolume();
        }
    }
#endif
    mIsPaused = false;
