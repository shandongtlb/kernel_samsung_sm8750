...
// SEC_GENERIC
            ...
            {SecParameters::kBleHeadsetNrec, SecParameters::Feature_SEC::SEC_GENERIC},
...
void SecModulePrimary::onSetSECGenericParameters(const std::vector<VendorParameter>& params) {
    for (const auto& param : params) {
        ...
        } else if (SecParameters::kBleHeadsetNrec == param.id) {
            bool isBleNrecOn = (paramValue == AUDIO_PARAMETER_VALUE_ON);
            LOG(INFO) << __func__ << " BLE NREC mode = "
                                  << (isBleNrecOn ? AUDIO_PARAMETER_VALUE_ON : AUDIO_PARAMETER_VALUE_OFF);
            mPlatform.setBleNrecState(isBleNrecOn);
        }
#endif
...
void SecModulePrimary::onSetSECTelephonyParameters(const std::vector<VendorParameter>& parameters) {
    ...
    for (const auto& p : parameters) {
        ...
        if (SecParameters::kVoiceCallState == p.id) {
            ...
            bool ringbackState = mPlatform.getRingbacktone();
            if (paramValue == AUDIO_PARAMETER_VALUE_ON) {
                ringbackState = true;
            } else if (paramValue == AUDIO_PARAMETER_VALUE_OFF) {
                ringbackState = false;
            }
            if (mPlatform.getRingbacktone() != ringbackState) {
                mPlatform.setRingbacktone(ringbackState);
