...
#ifdef SEC_AUDIO_EARLYDROP_PATCH
        if (mPlatform.getBLECaptureEnable()) {
            return AudioDevice{.type.type = AudioDeviceType::IN_HEADSET,
...
bool Telephony::isValidVSID(VSID vsid)
{
    if (vsid == VSID::VSID_1 ||
        vsid == VSID::VSID_2)
        return true;
...
void Telephony::updateSecVSID(const int vsid) {
    ...
    newUpdates.mVSID = VSID::VSID_INIT;
    ...
    if (!isValidVSID(newUpdates.mVSID)) {
        LOG(ERROR) << __func__ << ": invalid vsid " << newUpdates.mVSID;
        return;
    }
    ...
    if ((mSetUpdates.mCallState == CallState::ACTIVE && mPalHandle != nullptr)
        ...
        if (mCurVSID != newUpdates.mVSID) {
            mCurVSID = newUpdates.mVSID;
            LOG(DEBUG) << __func__ << ": mCurVSID to " << mCurVSID;
        }
    }
}
...
void Telephony::configureMicMode() {
    ...
    } else if (mPlatform.getDexConnected() || mPlatform.getDexPadConnected() || needToSetStandard) {
