...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
static int sde_dbg_reg_base_mdnie_open(struct inode *inode, struct file *file)
{
	...
	base_name[strlen(file->f_path.dentry->d_iname) - 15] = '\0';
	reg_base = _sde_dump_get_blk_addr(base_name);
	if (!reg_base) {
		pr_err("error: unable to locate base %s\n",
				base_name);
...
static ssize_t sde_dbg_reg_base_mdnie_reg_read(struct file *file,
			char __user *user_buf, size_t count, loff_t *ppos)
{
	struct sde_dbg_reg_base *dbg;
	size_t len;
	...
	if (!sde_crtc || !sde_crtc->mixers[0].hw_dspp || !sde_crtc->mixers[0].hw_dspp->cap || !sde_crtc->mixers[0].hw_dspp->cap->sblk) {
		pr_err("invalid sde_crtc value\n");
		return -EOPNOTSUPP;
	}

	if (!sde_crtc->mixers[0].hw_dspp->cap->sblk->aiqe.mdnie_supported) {
		pr_err("AIQE MDNIE not supported");
		return -EOPNOTSUPP;
	}
	...
	if (!dbg->buf) {
        ...
        dbg->off = sde_crtc->mixers[0].hw_dspp->hw.blk_off + sde_crtc->mixers[0].hw_dspp->cap->sblk->aiqe.base + 0x100;
		dbg->cnt = 4*(AIQE_MDNIE_PARAM_LEN + 1);
...
static const struct file_operations sde_aiqe_mdnie_reg_fops = {
	.open = sde_dbg_reg_base_mdnie_open,
	.release = sde_dbg_reg_base_release,
	.read = sde_dbg_reg_base_mdnie_reg_read,
};
#endif
...
int sde_dbg_debugfs_register(struct device *dev)
{
	...
	debugfs_create_file("sde_aiqe_mdnie_reg", 0400, debugfs_root, NULL, &sde_aiqe_mdnie_reg_fops);
#else
