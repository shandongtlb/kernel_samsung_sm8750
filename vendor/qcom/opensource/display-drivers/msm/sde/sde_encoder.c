			...
			enum sde_perf_commit_state commit_state)
{
	...
	ctl = cur_master->hw_ctl;
	if (commit_state == SDE_PERF_BEGIN_COMMIT && cur_master->connector) {
		drm_conn = sde_enc->cur_master->connector;
		ept = sde_connector_get_property(drm_conn->state, CONNECTOR_PROP_EPT);
		current_ts = ktime_get_ns();
		ept_ts = ept - (6 *  NSEC_PER_MSEC);

		if (ktime_compare(ept_ts, current_ts) <= 0) {
			SDE_DEBUG("enc:%d, ept elapsed; ept:%llu, ept_ts:%llu, current_ts:%llu\n",
					DRMID(&sde_enc->base), ept, ept_ts, current_ts);
			SDE_EVT32(DRMID(&sde_enc->base), ktime_to_us(current_ts),
					ktime_to_us(ept_ts), ept , SDE_EVTLOG_FUNC_CASE1);
		} else if (ktime_compare(ept_ts, current_ts) > 0) {
				timeout_us = DIV_ROUND_UP(ktime_sub(ept_ts, current_ts), 1000);
				SDE_DEBUG("enc:%d, ept elapsed; ept:%llu, ept_ts:%llu, current_ts:%llu timeout:%llu\n",
						DRMID(&sde_enc->base), ept, ept_ts, current_ts, timeout_us);
				SDE_EVT32(DRMID(&sde_enc->base), ktime_to_us(current_ts),
						ktime_to_us(ept_ts), ktime_to_us(timeout_us),ktime_to_us(ept) , SDE_EVTLOG_FUNC_CASE2);
...
void sde_encoder_vhm_wakelock(struct sde_encoder_virt *sde_enc, bool enable)
{
	...
	if (sde_kms->suspend_state) {
		SDE_EVT32(sde_enc->vrr_info.vhm_pm_wake_lock, enable);
		return;
	}

	if (enable && !sde_enc->vrr_info.vhm_pm_wake_lock) {
		sde_enc->vrr_info.vhm_pm_wake_lock = true;
		sde_power_wakelock_ctrl(&priv->phandle , true);
	} else if (!enable && sde_enc->vrr_info.vhm_pm_wake_lock){
		sde_power_wakelock_ctrl(&priv->phandle , false);
		sde_enc->vrr_info.vhm_pm_wake_lock = false;
	}
}
...
static unsigned int _sde_encoder_vrr_min_idle_time(struct sde_encoder_virt *sde_enc)
{
	...
	if (sde_enc->crtc && sde_enc->crtc->state->active_changed)
        vrr_min_idle_time_ms += nominal_vsync_ms;
	...
	u32 sw_event)
{
	...
	struct msm_drm_private *priv;
	unsigned int lp, idle_pc_duration, vrr_min_idle_time = 0, frame_time_ms, fps;
...
{
	if (!sde_enc->off_work_in_progress && (!sde_enc->crtc->state->active || _sde_encoder_is_autorefresh_enabled(sde_enc) ||
	...
	u32 sw_event, struct sde_encoder_virt *sde_enc, bool is_vid_mode)
{
	...
	SDE_DEBUG("rc_state = SDE_ENC_RC_STATE_ON\n");
	...
	u32 sw_event, struct sde_encoder_virt *sde_enc)
{
	...
	sde_enc->rc_state = SDE_ENC_RC_STATE_OFF;
	SDE_ERROR("rc_state = SDE_ENC_RC_STATE_OFF\n");
	...
	u32 sw_event, struct sde_encoder_virt *sde_enc, bool is_vid_mode)
{
	...
	if (sde_conn->panel_dead) {
		...
		goto end;
	} else if (mdnie_art_in_progress(&sde_crtc->aiqe_top_level) || sde_crtc->mdnie_ipc_disabled) {
		SDE_INFO("idle entry: mdnie_ipc (%d) crtc:%d \n",
			sde_crtc->mdnie_ipc_disabled, DRMID(crtc));
		sde_crtc->disable_art = true;
		SDE_EVT32(DRMID(drm_enc), sw_event, sde_enc->rc_state,
			sde_crtc->mdnie_ipc_disabled, SDE_EVTLOG_ERROR);
	}
	...
	sde_enc->rc_state = SDE_ENC_RC_STATE_IDLE;
	SDE_DEBUG("rc_state = SDE_ENC_RC_STATE_IDLE\n");
		...
		struct sde_encoder_virt *sde_enc, struct msm_display_mode *msm_mode)
{
	...
	for (i = 0; i < sde_enc->num_phys_encs; i++) {
		...
		phys->pclk_ratio = mult_frac(100, comp_info->dsc_info.transfer_rate_bpp, comp_info->dsc_info.config.bits_per_pixel >> 4);
...
static void sde_encoder_off_work(struct kthread_work *work)
{
	...
	sde_enc->off_work_in_progress = true;

	sde_encoder_idle_request(drm_enc);

	sde_enc->off_work_in_progress = false;
	SDE_ATRACE_END("sde_encoder_off_work");
}

int sde_encoder_enter_idle(struct drm_encoder *drm_enc, bool idle_enable)
{
	...
	// flush any scheduled commits
	msm_atomic_flush_display_threads(priv);

	//wait for frame trasnfer
	ret = sde_encoder_wait_for_event(drm_enc,
		MSM_ENC_TX_COMPLETE);

	if (ret && ret != -EWOULDBLOCK) {
		SDE_ERROR(
			"enter_idle wait for commit done returned %d\n", ret);
	} else if (!ret) {
		//enter idle
		...
		kthread_flush_worker(&priv->disp_thread[sde_enc->crtc->index].worker);
	}
	...
	for (i = 0; i < sde_enc->num_phys_encs; i++) {
		...
		if (sde_enc->disp_info.vrr_caps.vrr_support) {
			if (phys->ops.control_empulse_irq)
				phys->ops.control_empulse_irq(phys, enable, true);
		} else {
...
static void sde_encoder_wait_for_esync_vsync(struct sde_encoder_phys *phys)
{
	...
	SDE_EVT32(empulse_notification_sim, empulse_irq_refcount);
	if (empulse_notification_sim
				&& empulse_irq_refcount == 1) {
		/* Remove sim refcount*/
		if (phys->ops.control_empulse_irq)
			phys->ops.control_empulse_irq(phys, false, false);
		if (phys->ops.control_empulse_irq)
			phys->ops.control_empulse_irq(phys, true, false);
	} else if (empulse_irq_refcount > 1)
	...
	if (phys->ops.control_empulse_irq)
		phys->ops.control_empulse_irq(phys, false, true);
}
...
void sde_encoder_complete_commit(struct drm_encoder *drm_enc)
{
	...
	struct sde_hw_intf *hw_intf;
	u64 em_timestamp_hw = 0, vsync_counter = 0, panel_vsync_counter = 0;
	...
	if (phys_enc && phys_enc->hw_intf) {
		...
		if (hw_intf->ops.get_vsync_timestamp)
			vsync_counter = hw_intf->ops.get_vsync_timestamp(
					hw_intf, true);
		if (hw_intf->ops.get_panel_vsync_timestamp)
			panel_vsync_counter = hw_intf->ops.get_panel_vsync_timestamp(
					hw_intf);
		if (hw_intf->ops.get_esync_timestamp)
			em_timestamp_hw = hw_intf->ops.get_esync_timestamp(hw_intf);
		if (hw_intf->ops.get_status)
			hw_intf->ops.get_status(hw_intf, &intf_status);

		if (panel_vsync_counter >= em_timestamp_hw) {
			diff_emsync_vsync = panel_vsync_counter - em_timestamp_hw;
			diff_emsync_vsync_ns = ((diff_emsync_vsync * 10000) / 192);
		}

		SDE_EVT32(intf_status.frame_count, intf_status.line_count,
			vsync_counter >> 32, vsync_counter,
			panel_vsync_counter >> 32, panel_vsync_counter,
			em_timestamp_hw >> 32, em_timestamp_hw, diff_emsync_vsync_ns);

		if (diff_emsync_vsync_ns > ESYNC_EMSYNC_TO_PANEL_VSYNC_NS) {
			SDE_ERROR("mipi data and esync not modulated vc:%llu pvc:%llu emc:%llu diff:%llu\n",
				vsync_counter, panel_vsync_counter, em_timestamp_hw, diff_emsync_vsync_ns);
		...
		bool config_changed)
{
	...
	if (sde_enc->disp_info.fence_sw_override_always)
		ctl->ops.hw_fence_trigger_sw_override(ctl);
	...
	if (sde_enc->disp_info.vrr_caps.video_mrr_support
		...
		phys->ops.is_master && phys->ops.is_master(phys))
		...
		bool send_still_cmd)
{
	...
	SDE_EVT32(phys_enc->hw_intf->ops.get_line_count(phys_enc->hw_intf),
		phys_enc->pf_time_in_us, phys_enc->cached_mode.vtotal);
	...
	if (send_still_cmd) {
		...
		sde_encoder_vhm_wakelock(sde_enc, false);
		kthread_cancel_delayed_work_sync(&sde_enc->backlight_sr_work);
		if (debug_still_cmd_pending) {
			for (i = 0; i <3; i++) {
				SDE_EVT32(i);
				if ((i == 2) && debug_still_cmd_pending) {
					pr_err("Still indication cmd failure\n");
				}
				if (debug_still_cmd_pending)
					msleep(25);
				else
...
void sde_encoder_handle_self_refresh_video_psr(struct sde_encoder_phys *phys_enc, u32 new_commit)
{
	...
	if (sde_conn && sde_conn->bl_vrr.bl_increment_in_progress) {
		...
		if (ktime_compare(backlight_sr_timer_expires, current_time) > 0) {
			...
			if (ktime_to_ns(diff) < 2 * NSEC_PER_MSEC) {
				...
				hrtimer_start(&phys_enc->sde_vrr_cfg.backlight_timer,
						ns_to_ktime(NSEC_PER_MSEC * 5), HRTIMER_MODE_REL);
			}
		}
	}
	...
	if (sde_enc->disp_info.vrr_caps.video_mrr_support)
		avr_step_in_ns = SEC_TO_NS/240;
	else
		avr_step_in_ns = SEC_TO_NS/sde_enc->mode_info.avr_step_fps;
	dpu_min_trigger = dpu_min_ns - avr_step_in_ns + DEVIATION_NS;
	SDE_EVT32(dpu_min_trigger>>32, dpu_min_trigger, vrr_cfg->curr_freq_pattern->frame_interval,
		vrr_cfg->curr_freq_pattern->freq_stepping_seq[0], dpu_min_ns, sde_enc->mode_info.avr_step_fps, avr_step_in_ns);
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	/* Not required for CMD mode(MRR) panel */
	if (is_cmd_mode)
		return;
#endif
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	/* Shift ept value right more 500us than QC original */
	ept_ts = avr_step_fps ? ept - DIV_ROUND_UP(NSEC_PER_SEC, avr_step_fps) + NSEC_PER_MSEC + (NSEC_PER_MSEC >> 1)
...
u32 sde_encoder_helper_get_kickoff_timeout_ms(struct drm_encoder *drm_enc)
{
	...
	if (sde_enc->disp_info.vrr_caps.video_mrr_support)
		return (DEFAULT_KICKOFF_TIMEOUT_MS * 5);
	else
		fps = sde_enc->mode_info.frame_rate;
...
int sde_encoder_prepare_commit(struct drm_encoder *drm_enc)
{
	...
	if (sde_enc->cur_master) {
		if (sde_enc->rc_state != SDE_ENC_RC_STATE_ON) {
			sde_conn_state = to_sde_connector_state(sde_enc->cur_master->connector->state);
			sde_connector_set_private_flag(sde_conn_state, MSM_MODE_FLAG_SEAMLESS_DYN_CLK, false);
		}
