...
static void sde_hw_intf_prepare_esync(struct sde_hw_intf *ctx, struct intf_esync_params *params)
{
	...
	val = SDE_REG_READ(c, INTF_ESYNC_EN);
	val_ts_ctl = SDE_REG_READ(c, INTF_ESYNC_TIMESTAMP_CTRL);
	if (val & BIT(0))
		SDE_REG_WRITE(c, INTF_ESYNC_TIMESTAMP_CTRL, BIT(0) | BIT(2));

	val = SDE_REG_READ(c, INTF_ESYNC_EN);
	val_ts_ctl = SDE_REG_READ(c, INTF_ESYNC_TIMESTAMP_CTRL);
	if (!(val_ts_ctl & BIT(2)) && (val & BIT(0)))
		SDE_REG_WRITE(c, INTF_ESYNC_TIMESTAMP_CTRL, BIT(0) | BIT(2));

	val_ts_ctl = SDE_REG_READ(c, INTF_VSYNC_TIMESTAMP_CTRL);
	if (!(val_ts_ctl & BIT(4)) && (val & BIT(0)))
		SDE_REG_WRITE(c, INTF_VSYNC_TIMESTAMP_CTRL, BIT(0) | BIT(4));
}
		...
		const struct sde_format *fmt, bool align_esync, bool align_avr)
{
	...
	SDE_INFO("V start,end (%d,%d) vsync_pulse_width %d\n", 
			display_v_start, display_v_end, p->vsync_pulse_width);
		...
		unsigned long cap, unsigned long mdss_cap)
{
	...
	if (cap & (BIT(SDE_INTF_PANEL_VSYNC_TS) | BIT(SDE_INTF_MDP_VSYNC_TS))) {
		...
		ops->get_panel_vsync_timestamp = sde_hw_intf_get_panel_vsync_timestamp;
	}
