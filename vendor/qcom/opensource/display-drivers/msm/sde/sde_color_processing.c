...
static void _sde_cp_check_aiqe_properties(struct drm_crtc *crtc, struct sde_cp_node *prop_node)
{
	...
	case SDE_CP_CRTC_DSPP_MDNIE_IPC:
		...
		SDE_INFO("MDNIE: mdnie_ipc_disabled %d\n", sde_crtc->mdnie_ipc_disabled);
	...
	if (feature == FEATURE_MDNIE_ART) {
		if (prop_val) {
			...
			if (blob) {
				art = blob->data;
				if (sde_crtc->mdnie_ipc_disabled == false || sde_crtc->disable_art == true) {
						DRM_INFO("MDNIE: do not enable ART (mdine_ipc_disabled(%d) / disable_art(%d) \n",
								sde_crtc->mdnie_ipc_disabled, sde_crtc->disable_art);
						// disable ART in params
						art->param = 0;
						// set frame count as 1 to trigger ART done event
						sde_crtc->mdnie_art_frame_count = 1;
						SDE_EVT32(prop_node->feature, sde_crtc->mdnie_ipc_disabled);
						if (sde_crtc->disable_art == true)
							sde_crtc->disable_art = false;
						return;
				}
				aiqe_register_client(feature, &sde_crtc->aiqe_top_level);
				get_mdnie_art_frame_count(&sde_crtc->mdnie_art_frame_count,
							art->param);
				SDE_INFO("MDNIE: Enabled ART. mdnie_art_frame_count (%d) / art->param (%d)\n", sde_crtc->mdnie_art_frame_count, art->param);
				SDE_EVT32(prop_node->feature, sde_crtc->mdnie_ipc_disabled);
			}
		} else {
			aiqe_deregister_client(feature, &sde_crtc->aiqe_top_level);
			SDE_INFO("MDNIE: Disabled ART.\n");
		}
