...
static void _sde_aiqe_ssrc_config_off_v1(struct sde_hw_dspp *ctx,
	struct sde_hw_cp_cfg *hw_cfg, struct sde_aiqe_top_level *aiqe_top)
{
	u32 aiqe_base = ctx->cap->sblk->aiqe.base;

	sde_setup_aiqe_common_v1(ctx, hw_cfg, aiqe_top);
	SDE_REG_WRITE(&ctx->hw, aiqe_base + 0x380, 0);

	LOG_FEATURE_OFF;
}

void sde_setup_aiqe_ssrc_config_v1(struct sde_hw_dspp *ctx, void *cfg, void *aiqe_top)
{
	struct drm_msm_ssrc_config *ssrc_config;
	struct sde_aiqe_top_level *aiqe_tl = aiqe_top;
	struct sde_hw_cp_cfg *hw_cfg = cfg;
	...
	ssrc_config = hw_cfg->payload;
	if (!ssrc_config || (ssrc_config->config[0] & BIT(0)) == 0) {
		_sde_aiqe_ssrc_config_off_v1(ctx, hw_cfg, aiqe_tl);
		return;
	}
	...
	for (i = 0; i < AIQE_SSRC_PARAM_LEN; i++) {
		SDE_REG_WRITE(&ctx->hw, aiqe_base + 0x380 + (i * sizeof(u32)), ssrc_config->config[i]);
	}

	LOG_FEATURE_ON;
}

void sde_setup_aiqe_ssrc_data_v1(struct sde_hw_dspp *ctx, void *cfg, void *aiqe_top)
{
	struct drm_msm_ssrc_data *ssrc_data;
	struct sde_hw_cp_cfg *hw_cfg = cfg;
	int rc = -EINVAL;
	...
	while (index < ssrc_data->data_size) {
		...
		SDE_REG_WRITE(&ctx->hw, aiqe_base + 0x0C, ssrc_data->data[index]);

		for (size_t i = 0; i < (region_size - 1); i++) {
			SDE_REG_WRITE(&ctx->hw, aiqe_base + 0x10 , ssrc_data->data[index + 1 + i]);
		}
