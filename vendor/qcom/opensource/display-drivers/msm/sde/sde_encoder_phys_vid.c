		...
		struct intf_timing_params *timing)
{
	...
	if ((phys_enc->hw_intf->cap->type != INTF_DP) &&
		...
		comp_ratio = drm_fixp_from_fraction(phys_enc->pclk_ratio, 100);
	...
	struct sde_encoder_phys *phys_enc, bool exit_idle)
{
	...
	u32 hblank, avr_step_fps, vrefresh;
	...
	hblank = cached_mode->htotal - cached_mode->hdisplay;
	active_compressed = mult_frac(cached_mode->hdisplay, 100, phys_enc->pclk_ratio);
	...
	SDE_EVT32(cached_mode->vtotal, esync_params.avr_step_lines, avr_step_fps,
	...
	struct sde_encoder_phys *phys_enc, bool align)
{
	...
	active_compressed = mult_frac(phys_enc->cached_mode.hdisplay, 100, phys_enc->pclk_ratio);
	...
	backup_params->avr_step_lines = esync_params.avr_step_lines;
	SDE_EVT32(backup_params->backup_mode.htotal, backup_params->backup_mode.vtotal,
		...
		u32 vsync_source, struct msm_display_info *disp_info)
{
	...
	if ((sde_enc->disp_info.vrr_caps.video_psr_support || phys_enc->has_intf_te) &&
...
static void sde_encoder_phys_vid_esync_vsync_irq(void *arg, int irq_idx)
{
	...
	u64 vsync_counter = 0, em_timestamp_hw = 0, panel_vsync_counter = 0;
	...
	sde_enc = to_sde_encoder_virt(phys_enc->parent);
	if (sde_enc && sde_enc->cur_master && sde_enc->cur_master->hw_intf) {
	...
	SDE_EVT32_IRQ(DRMID(phys_enc->parent), irq_idx,
		...
		em_timestamp_hw >> 32, em_timestamp_hw,
...
not_flushed:
	...
	if (phys_enc && phys_enc->hw_intf && sde_enc->disp_info.vrr_caps.video_psr_support) {
		...
		if (diff_emsync_vsync_ns > 100)
			SDE_EVT32(vsync_counter >> 32, vsync_counter,
				panel_vsync_counter >> 32, panel_vsync_counter,
	...
	struct sde_encoder_phys *phys_enc, bool enable, bool acquire_lock)
{
	...
	if (info->vrr_caps.video_mrr_support)
		intr_idx = INTR_IDX_ESYNC_VSYNC;
	else
		intr_idx = INTR_IDX_ESYNC_EMSYNC;
	...
	if (enable && atomic_inc_return(&phys_enc->empulse_irq_refcount) == 1) {
		if (sde_enc->rc_state == SDE_ENC_RC_STATE_ON) {
			ret = sde_encoder_helper_register_irq(phys_enc, intr_idx);
			if (ret)
		...
		if (phys_enc->empulse_notification_sim) {
			...
			ret = sde_encoder_helper_unregister_irq(phys_enc, intr_idx);
			if (ret)
		...
		struct sde_encoder_phys *phys_enc, bool notify)
{
	...
	if (ret == -ETIMEDOUT) {
		...
		if (phys_enc->hw_intf->cap->type == INTF_DP)
			LCD_ERR_NOVDD("[SDE] INTF_DP, skip\n");
		else
			SDE_DBG_DUMP_WQ(SDE_DBG_BUILT_IN_ALL, "panic");
#endif
		new_cnt = atomic_add_unless(&phys_enc->pending_kickoff_cnt, -1, 0);
