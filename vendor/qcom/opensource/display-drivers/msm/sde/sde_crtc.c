...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
static ssize_t aiqe_license_status_show(struct device *device,
	struct device_attribute *attr, char *buf)
{
	struct drm_crtc *crtc;
	...
	return scnprintf(buf, PAGE_SIZE, "%s\n",sde_kms->catalog->ssip_allowed ? "Enable" : "Disable");
}
static DEVICE_ATTR_RO(aiqe_license_status);
#endif
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	&dev_attr_aiqe_license_status.attr,
#endif
	NULL
};
...
static void _sde_crtc_program_lm_output_roi(struct drm_crtc *crtc)
{
	...
	for (lm_idx = 0; lm_idx < sde_crtc->num_mixers; lm_idx++) {
		...
		SDE_EVT32(DRMID(crtc), sde_crtc->ai_scaler_res.enabled,sde_crtc->ai_scaler_res.src_w,
				sde_crtc->ai_scaler_res.src_h,sde_crtc->ai_scaler_res.dst_w,sde_crtc->ai_scaler_res.dst_h);
	}
...
void sde_crtc_copr_status_event_notify(struct drm_crtc *crtc)
{
	...
	struct panel_event_copr_data* copr_stat = &evt_data.d.copr;
	int i;
    u32 val_rg, val_bw;
	bool updated;
#endif
...
#if IS_ENABLED(CONFIG_SEC_PANEL_NOTIFIER_V2)
	updated = false;
	for (i = 0; i < COPR_STATUS_LEN; i++) {
		if (sde_crtc->prev_copr_status[i] != copr_status.status[i]){
			updated = true;
			break;
		}
	}
	...
	if (updated) {
		SDE_DEBUG("SS COPR Notify:+++");
		for (i = 0; i < MAX_COPR_ROI; i++) {
			val_rg = copr_status.status[2 * i];
			val_bw = copr_status.status[2 * i + 1];

			copr_stat->stat[i][AIQE_COPR_WRGB_G] = (val_rg & COPR_ROI_G_MASK) >> COPR_ROI_G_SHIFT;
			copr_stat->stat[i][AIQE_COPR_WRGB_R] = (val_rg & COPR_ROI_R_MASK) >> COPR_ROI_R_SHIFT;
			copr_stat->stat[i][AIQE_COPR_WRGB_W] = (val_bw & COPR_ROI_W_MASK) >> COPR_ROI_W_SHIFT;
			copr_stat->stat[i][AIQE_COPR_WRGB_B] = (val_bw & COPR_ROI_B_MASK) >> COPR_ROI_B_SHIFT;

			SDE_DEBUG("Copr[%d]:%u,%u,%u,%u\n", i, copr_stat->stat[i][AIQE_COPR_WRGB_R],
																copr_stat->stat[i][AIQE_COPR_WRGB_G],
																copr_stat->stat[i][AIQE_COPR_WRGB_B],
																copr_stat->stat[i][AIQE_COPR_WRGB_W]);

			sde_crtc->prev_copr_status[2 * i] = val_rg;
			sde_crtc->prev_copr_status[2 * i + 1] = val_bw;
		}

		copr_stat->nr_stat = MAX_AIQE_COPR_ROI;
		evt_data.state = sde_crtc->copr_status_event_notify_enabled? PANEL_EVENT_COPR_STATE_ENABLED : PANEL_EVENT_COPR_STATE_DISABLED;
		evt_data.display_index = drm_crtc_index(crtc);
...
static void _sde_crtc_set_input_fence_timeout(struct sde_crtc_state *cstate)
{
	...
	// Avoid CP crash (P250414-00326)
	// CP crash when transaction doesn't occur withn 3s
	cstate->input_fence_timeout_ns /= 10; 
...
bool sde_crtc_is_power_on_frame(struct drm_crtc *crtc)
{
...

{
	...
	SDE_ATRACE_END("setup_mixers");

	SDE_ATRACE_BEGIN("kickoff_pending");
	...
	SDE_ATRACE_END("kickoff_pending");
	...
	SDE_ATRACE_BEGIN("apply_property");
	if (sde_kms_is_cp_operation_allowed(sde_kms))
		sde_cp_crtc_apply_properties(crtc);
	SDE_ATRACE_END("apply_property");
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	for(int i = 0; i < COPR_STATUS_LEN; i++){
		sde_crtc->prev_copr_status[i] = 0;
	}
#endif
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	struct sde_kms *sde_kms = _sde_crtc_get_kms(crtc_drm);
	if (!sde_kms || !sde_kms->catalog) {
		return -EINVAL;
	}

	en = sde_kms->catalog->ssip_allowed && en;
#endif
