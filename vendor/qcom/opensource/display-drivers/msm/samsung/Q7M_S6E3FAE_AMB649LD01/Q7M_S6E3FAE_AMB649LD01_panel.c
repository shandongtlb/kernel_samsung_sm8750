...
*/
#include "Q7M_S6E3FAE_AMB649LD01_panel.h"
//#include "Q7M_S6E3FAE_AMB649LD01_mdnie.h"
...
static u8 vividness_skin_tone_table[VIVIDNESS_MAX_IDX][VIVIDNESS_SKIN_TONE_SIZE] = {
	{0xFD, 0x10, 0x20, 0xFF, 0xF9, 0x60, 0xFF, 0x10, 0xF0, 0xFF, 0xFF, 0xF6}, /* default */
	{0xF1, 0x20, 0x40, 0xF7, 0xFA, 0x40, 0xFF, 0x10, 0xF7, 0xFF, 0xFF, 0xFA},
	{0xEC, 0x10, 0x40, 0xF0, 0xFC, 0x40, 0xFF, 0x10, 0xFF, 0xFF, 0xFF, 0xFF},
};
			...
			char *val, struct ss_cmd_desc *cmd)
{
	...
	if (i + 1 > cmd->tx_len) {
		LCD_ERR(vdd, "fail to find proper 0xXX position(%d, %zu)\n",
				i, cmd->tx_len);
		goto err_skip;
	}
			...
			char *val, struct ss_cmd_desc *cmd)
{
	...
	if (!glut_map) {
		LCD_DEBUG(vdd, "No glut_map\n");
		goto err_skip;
	}

	if (i + glut_map->col_size > cmd->tx_len) {
		LCD_ERR(vdd, "fail to find proper 0xXX position(%d, %d, %zu)\n",
...
static void parse_glut(struct samsung_display_driver_data *vdd)
{
	...
	if (ret)
		...
		LCD_INFO(vdd, "Parsing GLUT_OFFSET_NIGHT_DIM data from panel_data_file [%d][%d]\n",
			vdd->br_info.glut_offset_night_dim.row_size, vdd->br_info.glut_offset_night_dim.col_size);

	vdd->night_dim_max_lv = vdd->br_info.glut_offset_night_dim.row_size;
}
#endif

static int ss_ddi_flash_test(struct samsung_display_driver_data *vdd, char *buf)
{
	u8 *val;
	int i, len = 0;
	int res = 0;
	...
	if (test_mode->pass_val) {
		...
		if (val) {
			if (ss_send_cmd_get_rx(vdd, RX_FLASH_LOADING_CHECK, val) <= 0) {
				...
				if (val[0] == test_mode->pass_val[0]) // && (val[1] & ~(test_mode->pass_val[1])) == 0)
					res = true;
				if ((val[1] & 0x9F) == 0x00) //pass values: 0x00, 0x20, 0x40, 0x60
					res = true;
				else
					res = false;
...
static int ss_ssr_read(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO(vdd, "SSR = 0x%02X -> %s\n", ssr, pass ? "SSR_PASS" : "SSR_FAIL");
...
static int ss_pre_brightness(struct samsung_display_driver_data *vdd)
{
	...
	/* EL_ON can be toggled if vlin1 of normal's and HBM's are different */
...
void Q7M_S6E3FAE_AMB649LD01_HD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->h_buf = Q7M_S6E3FAE_AMB649LD01_PDF_DATA;
	vdd->h_size = sizeof(Q7M_S6E3FAE_AMB649LD01_PDF_DATA);
