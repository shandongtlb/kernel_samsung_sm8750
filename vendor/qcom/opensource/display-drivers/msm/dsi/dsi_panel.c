...
#endif
{
	...
	if (IS_ERR_OR_NULL(state)) {
		DSI_ERR("[%s] failed to set pin state, state is NULL\n", panel->name);
		return -EINVAL;
	}
#endif
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
#define MIPI_FAIL_RETRY_CNT 2
int dsi_panel_tx_cmd_set(struct dsi_panel *panel,
				int type, bool do_peripheral_flush)
#else
...
#endif
{
	...
	SDE_EVT32(type, state, count, do_peripheral_flush);
	...
	for (i = 0; i < count; i++) {
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
		while (retry < MIPI_FAIL_RETRY_CNT) {
#endif
		...
		if (retry) {
			if (retry == MIPI_FAIL_RETRY_CNT) {
				if (!vdd->panel_dead) {
					DSI_ERR("vdd->during_panel_recovery = %d\n", vdd->during_panel_recovery);

					/* If mipi RX is failed */
					if (cmds->ctrl_flags & DSI_CTRL_CMD_READ) {
						/* do panel recovery */
						if (!vdd->during_panel_recovery) {
							LCD_ERR(vdd, "[%s] MIPI RX timeout. set panel_recovery!\n", ss_get_cmd_name(type));
							vdd->need_panel_recovery = true;
							vdd->mipi_fail_recovery_cnt++;
						} else {
							// Do not recovery again, because it is a mipi fail that occurred during recovery.
							vdd->mipi_fail_recovery_fail_cnt++;
							SDE_DBG_DUMP(SDE_DBG_BUILT_IN_ALL, "panic");
						}

						ss_rlog(vdd->ndx, type, retry, vdd->need_panel_recovery);
					}
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	if (panel->host_config.ext_bridge_mode) {
		DSI_ERR("ex_bridge_mode is enabled. skip bl(%d) setting\n", bl_lvl);
		return 0;
	}
#else
	if (panel->host_config.ext_bridge_mode)
				  ...
				  struct dsi_parser_utils *utils)
{
	...
	priv_info->native_vfp = mode->v_front_porch;
				     ...
				     struct device_node *of_node)
{
	...
	qsync_caps->fence_sw_override_always =
		utils->read_bool(utils->data, "qcom,fence_sw_override_always");
				...
				struct dsi_parser_utils *utils)
{
	...
	for (i = 0; i < freq_step_list->count; i++) {
		DSI_INFO("usecaseIdx:%d FI:%d, AP_SR:%d Num freq steps:%d Total steps:%d %p\n",
			freq_step_list->freq_pattern[i].usecase_idx,
			freq_step_list->freq_pattern[i].frame_interval,
			freq_pattern[i].needs_ap_refresh, freq_step_list->freq_pattern[i].num_freq_steps,
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	"qcom,mdss-dsi-freq-step-pattern9-command",
	"qcom,mdss-dsi-freq-step-pattern10-command",
#endif
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	"qcom,mdss-dsi-freq-step-pattern9-command-state",
	"qcom,mdss-dsi-freq-step-pattern10-command-state",
#endif
...
static int dsi_panel_parse_dsc_params(struct dsi_display_mode *mode,
				struct dsi_parser_utils *utils, u8 transfer_rate_bpp)
{
	u32 data;
	int rc = -EINVAL;
	int intf_width;
	...
	priv_info->dsc.transfer_rate_bpp = transfer_rate_bpp;
				...
				bool trusted_vm_env)
{
	...
	if (is_panel_xr) {
		...
		strscpy(new_panel_name, panel->name, size);
		strlcat(new_panel_name, panel_type, size);
/*TBD : Need to add Suffix - _secondary for the secondary display*/
#if !IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
		panel->name = new_panel_name;
#endif
	}
			...
			int topology_override)
{
	...
	dsi_for_each_child_node(timings_np, child_np) {
		...
		rc = dsi_panel_parse_dsc_params(mode, utils, panel->host_config.bpp);
		if (rc) {
				    ...
				    struct dsi_host_config *config)
{
	...
	config->u.video_engine.native_vfp = mode->priv_info->native_vfp;
...

{
	...
	for (i = 0; i < set->count; i++) {
		...
		SDE_EVT32(set->cmds[i].msg.flags);
	}
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	if (!panel->esync_caps.esync_support)
		ss_panel_off_pre(panel->panel_private); /* Video & Command Panel */
#endif
