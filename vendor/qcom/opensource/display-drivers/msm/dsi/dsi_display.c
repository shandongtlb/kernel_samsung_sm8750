			  ...
			  bool do_peripheral_flush)
{
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	vdd = display->panel->panel_private;
	/* SAMSUNG : Avoid sending DCS commands when Real ESD recovery is pending */
	if (atomic_read(&display->panel->esd_recovery_pending) && vdd->real_esd) {
#else
	...
	if (cmd->ctrl_flags & DSI_CTRL_CMD_BROADCAST) {
		...
		if (((cmd->ctrl_flags & DSI_CTRL_CMD_READ) && rc <= 0) ||
			...
			LCD_ERR(vdd, "cmd_transfer failed, rc=%d, cmd_flags=%dx\n",
				   rc, cmd->ctrl_flags);
			rc = -EINVAL;
		}
#else
				    ...
				    struct dsi_display_mode *mode, bool cts)
{
	...
	u64 pix_clk_rate = dsi_ctrl->clk_freq.pix_clk_rate;
	u64 esc_clk_rate = dsi_ctrl->clk_freq.esc_clk_rate;

	#define TO_ESC_CLK(val) ((val) * esc_clk_rate / pix_clk_rate)

	bool split = false; // FIXME:

	u32 line_time_pix = dsi_h_total_dce(&mode->timing);
	u32 line_time_esc = TO_ESC_CLK(line_time_pix);

	u32 pipe_delay_esc = line_time_esc;
	u32 pipe_delay2_esc = cts ? 20 : 0;
	u32 pipe_delay3_esc = cts ? 20 : 0;
	u32 pll_reg_flush_delay_esc = 50;
	u32 pll_reg_post_flush_delay_esc = 20;
	u32 min_pll_lock_time_esc = 480;

	u32 delay_start_dr_esc_comp = 3;
	u32 delay_start_dr_pix_comp = 1;
	u32 mdp_stop_deassertion_esc_comp = 1;
	u32 mdp_stop_deassertion_pix_comp = 3;
	u32 mask_clkln_fsm_delay_esc = cts ? 0 : 1;
	u32 delay_end_dr_pix = split ? 18 + 5 : 12 + 5;

	u32 min_dr_duration_pix_comp = delay_start_dr_pix_comp + delay_end_dr_pix +
		mdp_stop_deassertion_pix_comp;
	u32 min_dr_duration_esc_comp = delay_start_dr_esc_comp + pll_reg_flush_delay_esc +
	...
	u32 min_dr_duration_esc = min_dr_duration_esc_comp + TO_ESC_CLK(min_dr_duration_pix_comp);
	 ...
	 */
	u32 pll_delay_esc = min_dr_duration_esc +
					...
					struct link_clk_freq *bkp_freq)
{
	...
	display_for_each_ctrl(i, display) {
		...
		SDE_EVT32(i,ctrl->ctrl->clk_freq.pix_clk_rate, ctrl->ctrl->clk_freq.byte_clk_rate);
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (ctrl == m_ctrl)
			continue;
		dsi_phy_dynamic_refresh_trigger(ctrl->phy, false, display->panel->esync_caps.esync_support);
	}
	dsi_phy_dynamic_refresh_trigger(m_ctrl->phy, true, display->panel->esync_caps.esync_support);
				    ...
				    u32 flags)
{
	...
	if (mode->dsi_mode_flags &
		...
		display_for_each_ctrl(i, display) {
			...
			/* Avoid setting trigger selection for 2.9 due to hardware bug */
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	LCD_INFO_NOVDD("dsi_display_dev_probe ++\n");
#endif
	...
	if (boot_disp->boot_disp_en) {
		...
		if (!panel_node)
			DSI_WARN("%s panel_node %s not found\n", display->display_type,
					boot_disp->name);
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
		LCD_ERR_NOVDD("[SDE] %s boot_disp_en True, boot_disp->name=%s\n", display->display_type, boot_disp->name);
		if (panel_node)
			samsung_panel_initialize(boot_disp->name, index);
		else {
			LCD_ERR_NOVDD("[SDE] Display Panel is not found. Failed to probe\n");
			rc = -EPROBE_DEFER;
			goto end;
		}
#endif
		...
		struct msm_display_info *info, void *disp)
{
	...
	info->fence_sw_override_always = display->panel->qsync_caps.fence_sw_override_always;
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
		DSI_INFO("[DynamicClock] new_htotal (%llu) bpp (%d) vtotal (%llu) refresh_rate (%d)\n",
			new_htotal, bpp, vtotal, dsi_mode->timing.refresh_rate);

		DSI_INFO("[DynamicClock] previous HFP (%d) old_htotal (%llu)\n",
			dsi_mode->timing.h_front_porch, old_htotal);
#endif
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
		DSI_INFO("[DynamicClock] HSA (%d) HBP (%d) HACT (%d) HFP (%d) new_htotal (%llu) clock (%llu)\n",
					dsi_mode->timing.h_sync_width, dsi_mode->timing.h_back_porch, dsi_mode->timing.h_active,
					dsi_mode->timing.h_front_porch, new_htotal, dsi_mode->timing.clk_rate_hz);
#endif
		break;
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
		DSI_INFO("[DynamicClock-HFP] HACT (%d) HSA (%d) HBP (%d) HFP (%d)\n",
					dsi_mode->timing.h_active, dsi_mode->timing.h_sync_width,
					dsi_mode->timing.h_back_porch, dsi_mode->timing.h_front_porch);
#endif
		break;
...
static void _dsi_display_populate_bit_clks(struct dsi_display *display, int start, int end)
{
	...
	int i, j, bpp, lanes = 0;
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG) && IS_ENABLED(CONFIG_PANEL_USE_VHM_DYN_MIPI_CLOCK)
	struct samsung_display_driver_data *vdd;
#endif
	...
	for (i = start; i < end; i++) {
		...
		vdd = display->panel->panel_private;
		src->timing.clk_rate_hz = src->priv_info->bit_clk_list.rates[vdd->dyn_mipi_clk.default_idx];
		DSI_INFO("set default src->timing.clk_rate_hz = %llu \n", src->timing.clk_rate_hz);
#endif
	...
	if (mode->priv_info->bit_clk_list.count) {
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG) && IS_ENABLED(CONFIG_PANEL_USE_VHM_DYN_MIPI_CLOCK)
		clk_rate_hz = mode->priv_info->bit_clk_list.rates[vdd->dyn_mipi_clk.default_idx];
		front_porch = mode->priv_info->bit_clk_list.front_porches[vdd->dyn_mipi_clk.default_idx];
#endif
		for (i = 0; i < mode->priv_info->bit_clk_list.count; i++) {
			if (display->dyn_bit_clk == mode->priv_info->bit_clk_list.rates[i]) {
				...
				DSI_DEBUG("dyn_bit_clk:%u, mode->priv_info->bit_clk_list.rates[%d]:%u, front_porch : %d\n",
					display->dyn_bit_clk, i, mode->priv_info->bit_clk_list.rates[i], front_porch);
			}
		}
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (is_skip_op_required(display)) {
		...
		ss_esync_on_post(display->panel->panel_private);
