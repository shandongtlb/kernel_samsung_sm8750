				     ...
				     struct dsi_host_config *host_config)
{
	...
	DSI_INFO("htotal = %d s,b,a,f (%d %d %d %d) / vtotal = %d s,b,a,f (%d %d %d %d)\n",
		h_total, mode->h_sync_width, mode->h_back_porch, mode->h_active, mode->h_front_porch,
		v_total, mode->v_sync_width, mode->v_back_porch, mode->v_active, mode->v_front_porch);
	...
	if (host_config->esync_enabled) {
		...
		DSI_W32(ctrl, DSI_VIDEO_MODE_CTRL5, active_v_end + host_config->u.video_engine.native_vfp);
